...
class FlutterDocScanner {
  ...
  ///
  /// [page] is the maximum number of pages to scan (must be >= 1, defaults to 10).
  /// Returns `null` if the user cancelled the scan.
  /// Throws [DocScanException] on failure.
  Future<dynamic> getScanDocuments({int page = 10}) {
    _validatePage(page);
  ...
  /// Returns `null` if the user cancelled the scan.
  /// Throws [DocScanException] on failure.
  Future<PdfScanResult?> getScannedDocumentAsPdf({int page = 10}) async {
    _validatePage(page);
    final data =
  ...
  Future<ImageScanResult?> getScanDocumentsUri({int page = 10}) async {
    _validatePage(page);
