  ...
  Future<void> syncLearnersFromServer(String classID) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint(
            '[SYNC] Received ${learnersData.length} learners from server');
        debugPrint(
            '[SYNC] First learner data sample: ${learnersData.isNotEmpty ? learnersData.first : 'No learners'}');
        ...
        if (learnersData.isNotEmpty) {
          for (int i = 0; i < math.min(3, learnersData.length); i++) {
            ...
            debugPrint(
                '[SYNC] Learner ${i + 1} (ID: ${learner['LearnerID']}) fingerprint fields:');
            debugPrint(
                '[SYNC]   fingerprint_template: ${learner['fingerprint_template']?.toString().length ?? 'null'} chars');
            debugPrint('[SYNC]   isLeftHand: ${learner['isLeftHand']}');
            debugPrint(
                '[SYNC]   zkteco_left_template: ${learner['zkteco_left_template']?.toString().length ?? 'null'} chars');
            debugPrint(
                '[SYNC]   zkteco_right_template: ${learner['zkteco_right_template']?.toString().length ?? 'null'} chars');
            debugPrint(
                '[SYNC]   futronic_left_template: ${learner['futronic_left_template']?.toString().length ?? 'null'} chars');
            debugPrint(
                '[SYNC]   futronic_right_template: ${learner['futronic_right_template']?.toString().length ?? 'null'} chars');
            debugPrint(
                '[SYNC]   sourceafis_template: ${learner['sourceafis_template']?.toString().length ?? 'null'} chars');
          }
        }
        ...
        for (var learner in learnersData) {
          try {
            ...
            debugPrint(
                '[SYNC] Successfully inserted learner: ${learnerData['Name']} ${learnerData['Surname']}');
