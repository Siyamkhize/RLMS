...
class FingerprintService {
  ...
  final _enrollSuccessController =
  ...
  Stream<Map<String, dynamic>> get enrollSuccessStream =>
      _enrollSuccessController.stream;
  ...
  bool get isBusy =>
      _isEnrolling || _isCapturing || _isChecking || _isVerifying;
      ...
      switch (call.method) {
        ...
        case 'onEnrollStatus':
          if (call.arguments is String) {
            ...
            if (status.toLowerCase().contains('success') ||
              ...
              _isEnrolling = false;
              _blockAllOperations =
                  false; // Clear block when enrollment completes
        ...
        case 'onVerifyResult':
          if (call.arguments is bool) {
            ...
            _verifyResultController
                .addError('Invalid verification result data');
          }
  ...
  Future<void> verifyFingerprint(
      {required String storedTemplate1, String? storedTemplate2}) async {
    if (isBusy) {
    ...
    _operationTimeout = Timer(const Duration(seconds: 15), () {
      if (_isVerifying) {
        ...
        _sensorStatusController
            .add('Verification timed out. Please try again.');
      }
    });
  ...
  Future<bool> isSensorConnected() async {
    ...
    try {
      final bool? isConnected =
          await _channel.invokeMethod('isSensorConnected');
      if (kDebugMode) {
  ...
  Future<void> forceResetAllStates() async {
    if (kDebugMode) {
      ...
      print(
          'Before reset: isEnrolling=$_isEnrolling, isCapturing=$_isCapturing, isChecking=$_isChecking, isVerifying=$_isVerifying, blocked=$_blockAllOperations');
    }
    ...
    if (kDebugMode) {
      print(
          'After reset: isEnrolling=$_isEnrolling, isCapturing=$_isCapturing, isChecking=$_isChecking, isVerifying=$_isVerifying, blocked=$_blockAllOperations');
  ...
  void manualReset() {
    if (kDebugMode) {
      ...
      print(
          'Before manual reset: isEnrolling=$_isEnrolling, isCapturing=$_isCapturing, isChecking=$_isChecking, isVerifying=$_isVerifying');
    }
    ...
    if (kDebugMode) {
      print(
          'After manual reset: isEnrolling=$_isEnrolling, isCapturing=$_isCapturing, isChecking=$_isChecking, isVerifying=$_isVerifying');
  ...
  Future<void> startEnrollment(String finger) async {
    debugPrint(
        '[ENROLL] FingerprintService.startEnrollment called for $finger');
    if (_isEnrolling) {
    ...
    try {
      ...
      debugPrint(
          '[ENROLL] FingerprintService.startEnrollment completed for $finger');
      if (kDebugMode) {
  ...
  Future<bool> matchTemplates(
      String storedTemplate, String scannedTemplate) async {
    try {
      ...
      if (kDebugMode) {
        print(
            'matchTemplates: stored length=${storedTemplate.length}, scanned length=${scannedTemplate.length}, match=$match');
      }
      return match ?? false;
    } on PlatformException catch (e) {
  ...
  Future<bool> verify(String finger, String template) async {
    return await _channel
        .invokeMethod('verify', {'finger': finger, 'template': template});
  }
  ...
  void dispose() {
    ...
    try {
      ...
      debugPrint('[FingerprintService] Error cancelling timeout: $e');
    }

    // Close streams only if they haven't been closed already
    try {
      ...
      debugPrint(
          '[FingerprintService] Error closing enrollStatusController: $e');
    }
    ...
    try {
      ...
      debugPrint(
          '[FingerprintService] Error closing enrollSuccessController: $e');
    }

    try {
      if (!_sensorStatusController.isClosed) {
        debugPrint('[FingerprintService] Closing sensorStatusController...');
        _sensorStatusController.close();
      }
    } catch (e) {
      debugPrint(
          '[FingerprintService] Error closing sensorStatusController: $e');
    }

    try {
      if (!_verifyResultController.isClosed) {
        debugPrint('[FingerprintService] Closing verifyResultController...');
        _verifyResultController.close();
      }
    } catch (e) {
      debugPrint(
          '[FingerprintService] Error closing verifyResultController: $e');
    }

    if (kDebugMode) {
      print('FingerprintService disposed successfully');
    }
  }
}
