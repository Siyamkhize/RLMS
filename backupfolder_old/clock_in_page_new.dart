...
class _ClockInPageState extends State<ClockInPage> {
  ...
  final Map<String, Map<String, dynamic>> _profileCompleteness = {};
                      ...
                      Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 8),
                            Text(
                              'Check which profile fields need to be completed',
                              style: TextStyle(
                                fontSize: 12,
                            ...
                            ElevatedButton.icon(
                              onPressed: () =>
                                  _showProfileCompletenessDialog(learnerId),
  ...
  Future<void> _initializeData() async {
    ...
    // Load profile completeness after learners are loaded
  ...
  void _filterLearners() {
    ...
    // Load profile completeness for filtered learners
    _loadProfileCompleteness();
  }

  // Profile completeness checker
  Future<Map<String, dynamic>> _checkLearnerProfileCompleteness(
      String learnerId) async {
    try {
      final response = await http.get(
        Uri.parse(AppConfig.buildUrl(
            'mobile/debug_learner_profile_completeness.php?learner_id=$learnerId')),
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true &&
            data['learners'] != null &&
            data['learners'].isNotEmpty) {
          return data['learners'][0];
        }
      }
    } catch (e) {
      print(
          '[PROFILE_COMPLETENESS] Error checking profile for learner $learnerId: $e');
    }

    // Return default incomplete profile if API fails
    return {
      'learner_id': learnerId,
      'completion_percentage': 0.0,
      'completed_fields_count': 0,
      'total_fields_count': 22,
      'incomplete_fields': [
        'Name',
        'Surname',
        'IDNumber',
  ...
  // Load profile completeness for all learners
  Future<void> _loadProfileCompleteness() async {
    if (!_isConnected) return; // Skip if offline

    for (final learner in _filteredLearners) {
      final learnerId = learner['LearnerID']?.toString();
      if (learnerId != null && !_profileCompleteness.containsKey(learnerId)) {
        final completeness = await _checkLearnerProfileCompleteness(learnerId);
        if (mounted) {
          setState(() {
            _profileCompleteness[learnerId] = completeness;
          });
        }
      }
    }
  ...
  void _showProfileUpdateDialog(
      String learnerId, Map<String, dynamic> completeness) {
    final incompleteFields =
        List<String>.from(completeness['incomplete_fields'] ?? []);
    final completionPercentage =
        (completeness['completion_percentage'] ?? 0.0).toDouble();
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text(
              'Profile Completion: ${completionPercentage.toStringAsFixed(1)}%'),
          content: SizedBox(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 16),
                Text(
                  'Missing Fields (${incompleteFields.length}):',
                  style: const TextStyle(fontWeight: FontWeight.bold),
                ),
                const SizedBox(height: 8),
                Flexible(
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                Navigator.of(context).pop();
                _navigateToLearnerDetails(learnerId);
              },
  ...
  // Navigate to learner details for profile update
  void _navigateToLearnerDetails(String learnerId) {
    final learner = _filteredLearners.firstWhere(
    ...
    if (learner != null) {
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (context) => LearnerDetailsPage(
            learnerID: learner['LearnerID']?.toString() ?? learnerId,
          ),
        ),
      ).then((_) {
        // Refresh profile completeness after returning from details page
        _profileCompleteness.remove(learnerId);
        _loadProfileCompleteness();
      });
    }
  }
  ...
  Future<void> _showProfileCompletenessDialog(String learnerId) async {
    ...
    if (_profileCompleteness.containsKey(learnerId)) {
      completeness = _profileCompleteness[learnerId]!;
    } else {
        ...
        builder: (context) => const AlertDialog(
          content: Row(
            ...
            children: [
              CircularProgressIndicator(),
              SizedBox(width: 16),
              Text('Checking profile completeness...'),
            ],
          ),
        ),
      );

      completeness = await _checkLearnerProfileCompleteness(learnerId);
    ...
    final completedFields =
        List<String>.from(completeness['completed_fields'] ?? []);
    final completionPercentage =
        (completeness['completion_percentage'] ?? 0.0).toDouble();
    final totalFields = completeness['total_fields_count'] ?? 22;
          ...
          content: SizedBox(
            ...
            child: Column(
              ...
              children: [
                ...
                // Tabs for completed and missing fields
                Expanded(
                    ...
                    child: Column(
                      children: [
                        TabBar(
                          ...
                          tabs: [
                            Tab(text: 'Missing (${incompleteFields.length})'),
                            Tab(text: 'Completed (${completedFields.length})'),
                          ],
                        ),
                        Expanded(
                          child: TabBarView(
                            children: [
                                  ...
                                  ? const Center(
                                      child: Column(
                                        ...
                                        children: [
                                          ...
                                          SizedBox(height: 8),
                                          Text('All profile fields completed!',
                                              style: TextStyle(
                                                  color: Colors.green)),
                                        ],
                                      ),
                              ...
                              // Completed fields tab
                              completedFields.isEmpty
                                  ? const Center(
                                      child: Text('No fields completed yet',
