...
class FingerprintErrorHandler {
  ...
  static String getFriendlyErrorMessage(String error) {
    ...
    if (lowerError.contains('capture_partial') ||
        lowerError.contains('partial')) {
      return 'Finger not placed properly. Please place your full thumb on the scanner.';
    }

    // Wrong finger errors
    if (lowerError.contains('no_match') ||
        lowerError.contains('verification_failed')) {
      return 'Fingerprint not recognized. Please try with your enrolled finger.';
    }

    // Connection errors
    if (lowerError.contains('usb_open_failed') ||
        lowerError.contains('device_open_failed')) {
      return 'Scanner not connected. Please check USB connection and try again.';
    }

    // Capture errors
    if (lowerError.contains('capture_failed') ||
        lowerError.contains('capture_error')) {
