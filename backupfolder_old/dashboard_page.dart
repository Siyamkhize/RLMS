...
class _DashboardPageState extends State<DashboardPage> {
  ...
  void initState() {
    ...
    print("[DASHBOARD] Initializing dashboard with classID: '${widget.classID}'");
    ...
    for (var learner in widget.learners) {
      final hasAnyFingerprint = (learner['zkteco_left_template']?.isNotEmpty == true) ||
    ...
    print("[DASHBOARD] ⚠️  Initial learners with fingerprints: $initialLearnersWithFingerprints");
    ...
    print("[DASHBOARD] ✅ Monitoring service started for class: ${widget.classID}");
  ...
  @override
  void dispose() {
    print('[DASHBOARD] ⚠️  DISPOSING DASHBOARD PAGE - This should only happen during intended logout!');
  ...
  void _initializeFingerprintServices() {
    ...
    _enrollSuccessSubscription = _fingerprintService.enrollSuccessStream.listen((capturedData) async {
  ...
  Future<void> reloadClassData({String? overrideClassID}) async {
    try {
      print("[DASHBOARD] Starting to reload class data for classID: ${widget.classID}");
      ...
      print("[DASHBOARD] About to call getLearnersWithClockingData for classID: $effectiveClassID");
      final learnersData = await DatabaseHelper().getLearnersWithClockingData(effectiveClassID);
      ...
      if (learnersData.isEmpty) {
        ...
        print("[DASHBOARD] Total learners in database: ${totalLearners.length}");
        ...
        if (totalLearners.isNotEmpty) {
          ...
          for (int i = 0; i < totalLearners.length && i < 5; i++) {
            ...
            print("[DASHBOARD]   ${learner['LearnerID']} (${learner['Name']} ${learner['Surname']}) - classID: ${learner['classID']}");
          }
          ...
          print("[DASHBOARD] Learners matching classID ${widget.classID}: ${matchingClassLearners.length}");
          
          if (matchingClassLearners.isEmpty) {
            // Check what classIDs exist
            final distinctClasses = await db.rawQuery('SELECT DISTINCT classID FROM learnerdetails WHERE classID IS NOT NULL');
            print("[DASHBOARD] Available classIDs in database: ${distinctClasses.map((e) => e['classID']).toList()}");
          }
        }
      }
      
      print("[DASHBOARD] Learners fetch completed, widget still mounted: $mounted");
      ...
      setState(() {
        ...
        print("[DASHBOARD] ✅ Reloaded learners with fingerprints: $reloadedLearnersWithFingerprints");
      }
    } catch (e) {
  ...
  Future<Map<String, dynamic>> _loadClassDataWithCorrectClassID(String correctClassID) async {
    try {
      print("[DASHBOARD] Loading class data with corrected classID: $correctClassID");
      
      // Fetch the original class data structure but with correct learners
      final data = await fetchClassData(widget.classID);
      final learnersData = await DatabaseHelper().getLearnersWithClockingData(correctClassID);
      ...
      print("[DASHBOARD] ✅ Corrected class data loaded with ${learnersData.length} learners");
  ...
  Future<Map<String, dynamic>> fetchClassData(String classID) async {
    try {
      print("[DASHBOARD] Attempting to fetch class data for classID: '$classID'");
      
      final result = await DatabaseHelper().fetchClassDetailsAndAttendance(classID);
  ...
  Future<void> _startFingerprintClocking() async {
    ...
    try {
      ...
      if (scanner == 'none') {
        _showErrorDialog('Scanner Not Found', 'No fingerprint scanner detected. Please connect a Futronic or ZKTeco scanner and try again.');
        return;
      }
  ...
  Future<void> _startFutronicScanning() async {
    try {
      ...
      _showErrorDialog('Futronic Error', 'Futronic scan failed: $e', skipPop: true);
    }
  }
  ...
  Future<void> _performFutronicOneToNVerification() async {
    ...
    _showMessage('Scanning completed! Verifying against class learners...', Colors.blue);
    ...
    try {
      ...
      print('[1:N] Starting OPTIMIZED FUTRONIC verification against ${currentLearners.length} learners');
      print('[1:N] ⚠️  DEBUG: currentLearners at verification time: ${currentLearners.length}');
      print('[1:N] ⚠️  DEBUG: First 3 learners: ${currentLearners.take(3).toList()}');
      ...
      for (final learner in currentLearners) {
        ...
        if ((futronicLeft != null && futronicLeft.toString().isNotEmpty) || 
          ...
          if (futronicLeft != null && futronicLeft.toString().length > 200) priority += 5;
          if (futronicRight != null && futronicRight.toString().length > 200) priority += 5;
      ...
      print('[1:N] Pre-filtered to ${candidateLearners.length} learners with Futronic templates');
      ...
      candidateLearners.sort((a, b) {
        ...
        int scoreA = 0;
        int scoreB = 0;
        if (a['leftTemplate'] != null && a['leftTemplate'].isNotEmpty) scoreA += 1;
        if (a['rightTemplate'] != null && a['rightTemplate'].isNotEmpty) scoreA += 1;
        if (b['leftTemplate'] != null && b['leftTemplate'].isNotEmpty) scoreB += 1;
        if (b['rightTemplate'] != null && b['rightTemplate'].isNotEmpty) scoreB += 1;
        
        return scoreB.compareTo(scoreA); // Higher scores first
      });
      
      // OPTIMIZATION 3: ULTRA-AGGRESSIVE - Limit to top 5 candidates for Futronic
      const maxFutronicCandidates = 5; // ULTRA-FAST: Only check top 5 most likely candidates
      final limitedFutronicCandidates = candidateLearners.take(maxFutronicCandidates).toList();
      print('[1:N] ⚡ FUTRONIC ULTRA-FAST: Processing only top ${limitedFutronicCandidates.length}/${candidateLearners.length} candidates');
      ...
      for (final candidate in limitedFutronicCandidates) {
        ...
        print('[1:N] ⚡ LIGHTNING-FAST checking learner: $learnerId ($learnerName)');
        print('[1:N]   Left: ${leftTemplate?.isNotEmpty == true ? "YES (${leftTemplate!.length})" : "NO"}');
        print('[1:N]   Right: ${rightTemplate?.isNotEmpty == true ? "YES (${rightTemplate!.length})" : "NO"}');
        ...
        try {
          ...
          if (leftTemplate != null && leftTemplate.isNotEmpty && 
              rightTemplate != null && rightTemplate.isNotEmpty) {
            // Best case: both templates available
            final hint = 'left'; // Always start with left when both available
            print('[1:N] FAST verifyBoth for learner $learnerId with hint: $hint');
            ...
            print('[1:N] FAST verifyBoth result for learner $learnerId: $match');
            ...
            print('[1:N] FAST verify (left) result for learner $learnerId: $match');
            ...
            print('[1:N] FAST verify (right) result for learner $learnerId: $match');
          }
          ...
          print('[1:N] Error in FAST Futronic verification for learner $learnerId: $e');
          continue;
        }
      }
      ...
      if (learnersWithTemplates == 0) {
        print('[1:N] ⚠️  SECURITY: No learners have Futronic templates in current class');
        
        // AUTO-FIX: Try to load learners from classID with fingerprints
        print('[1:N] 🔧 Attempting AUTO-FIX: Loading learners from correct classID...');
        final db = await DatabaseHelper().database;
        ...
        if (classQuery.isNotEmpty) {
          ...
          if (suggestedClassID != null) {
            print('[1:N] 🎯 Found classID $suggestedClassID with ${classQuery.first['learner_count']} Futronic templates');
            final learnersFromCorrectClass = await DatabaseHelper().getLearnersWithClockingData(suggestedClassID);
            
            if (learnersFromCorrectClass.isNotEmpty) {
              print('[1:N] ✅ AUTO-FIX SUCCESS: Loaded ${learnersFromCorrectClass.length} learners from correct classID');
              
              // PERMANENT FIX: Update widget classID and reload all data with correct classID
              print('[1:N] 🔧 PERMANENT FIX: Switching dashboard to use classID $suggestedClassID permanently');
              ...
              print('[1:N] ✅ AUTO-FIX COMPLETE: Dashboard now permanently uses correct classID $suggestedClassID');
              return;
            }
          }
        }
        
        print('[1:N] ❌ AUTO-FIX FAILED: No classID with Futronic templates found');
        _showVerificationFailedDialog(0, 'Futronic - No Templates Available');
        return;
      }
      
      if (matchedLearnerId != null && matchedLearnerName != null) {
        print('[1:N] ✅ SECURE MATCH: Proceeding with verified learner $matchedLearnerId');
        await _handleMatchedLearner(matchedLearnerId, matchedLearnerName, scanningDialogAlreadyClosed: true);
      } else {
      ...
      _showErrorDialog('Verification Error', 'Futronic verification failed: $e');
    }
  }
  ...
  Future<void> _performOneToNVerification(String scannedTemplate) async {
    ...
    try {
      ...
      print('[1:N] Starting ULTRA-FAST ZKTECO verification against ${currentLearners.length} learners');
      ...
      for (final learner in currentLearners) {
        ...
        if ((zktecoLeft != null && zktecoLeft.toString().isNotEmpty) || 
          ...
          // PRIORITY 3: Template size (better quality)
          if (zktecoLeft != null && zktecoLeft.toString().length > 200) priority += 5;
          if (zktecoRight != null && zktecoRight.toString().length > 200) priority += 5;
      ...
      print('[1:N] Pre-filtered to ${candidateLearners.length} learners with ZKTeco templates');
      ...
      candidateLearners.sort((a, b) {
        ...
        if (a['leftTemplate'] != null && a['leftTemplate'].isNotEmpty && a['leftTemplate'].length > 100) templateScoreA += 1;
        if (a['rightTemplate'] != null && a['rightTemplate'].isNotEmpty && a['rightTemplate'].length > 100) templateScoreA += 1;
        if (b['leftTemplate'] != null && b['leftTemplate'].isNotEmpty && b['leftTemplate'].length > 100) templateScoreB += 1;
        if (b['rightTemplate'] != null && b['rightTemplate'].isNotEmpty && b['rightTemplate'].length > 100) templateScoreB += 1;
      ...
      const maxCandidates = 5; // ULTRA-FAST: Only check top 5 most likely candidates
      ...
      print('[1:N] ⚡ TEMPLATE PRE-LOAD COMPLETE: ${preloadTime}ms for ${limitedCandidates.length}/${candidateLearners.length} candidates');
      ...
      for (final candidate in candidateLearners) {
        ...
        bool hasValidLeft = leftTemplate != null && leftTemplate.isNotEmpty && leftTemplate.length > 100;
        bool hasValidRight = rightTemplate != null && rightTemplate.isNotEmpty && rightTemplate.length > 100;
        
        print('[1:N] Template validation - Left: $hasValidLeft, Right: $hasValidRight');
        ...
        try {
          ...
          if (hasValidLeft && hasValidRight) {
            ...
            // Left template only
            print('[1:N] FAST ZKTeco verify (left only) for learner $learnerId');
            try {
              match = await _fingerprintService.verify('left', leftTemplate!);
              print('[1:N] FAST ZKTeco left verify result: $match');
            } catch (e) {
            ...
            // Right template only
            print('[1:N] FAST ZKTeco verify (right only) for learner $learnerId');
            try {
              match = await _fingerprintService.verify('right', rightTemplate!);
              print('[1:N] FAST ZKTeco right verify result: $match');
            } catch (e) {
          ...
          print('[1:N] Error in FAST ZKTeco verification for learner $learnerId: $e');
          continue;
        }
      }
      ...
      // SECURITY CHECK: Ensure we have templates before allowing any match
      if (learnersWithTemplates == 0) {
        print('[1:N] ⚠️  SECURITY: No learners have ZKTeco templates in current class');
        ...
        if (classQuery.isNotEmpty) {
          final suggestedClassID = classQuery.first['classID']?.toString();
          if (suggestedClassID != null) {
            print('[1:N] 🎯 Found classID $suggestedClassID with ${classQuery.first['learner_count']} ZKTeco templates');
            final learnersFromCorrectClass = await DatabaseHelper().getLearnersWithClockingData(suggestedClassID);
        ...
        print('[1:N] ❌ AUTO-FIX FAILED: No classID with ZKTeco templates found');
      ...
      if (matchedLearnerId != null && matchedLearnerName != null) {
        print('[1:N] ✅ SECURE MATCH: Proceeding with verified learner $matchedLearnerId');
        await _handleMatchedLearner(matchedLearnerId, matchedLearnerName, scanningDialogAlreadyClosed: false);
      } else {
  ...
  Future<void> _handleMatchedLearner(String learnerId, String learnerName, {bool scanningDialogAlreadyClosed = false}) async {
    print('[HANDLE_MATCHED] _handleMatchedLearner called for learner: $learnerId ($learnerName)');
    
    try {
      print('[HANDLE_MATCHED] Creating DatabaseHelper instance');
      final dbHelper = DatabaseHelper();
      
      print('[HANDLE_MATCHED] Getting attendance for day: ${_getCurrentDateString()}');
      final existingAttendance = await dbHelper.getAttendanceForDay(learnerId, _getCurrentDateString());
      ...
      if (existingAttendance == null || existingAttendance['clock_in_time'] == null) {
      ...
      } else if (existingAttendance['clock_out_time'] == null || existingAttendance['clock_out_time'].toString().isEmpty) {
        // Clock Out
        ...
        print('[HANDLE_MATCHED] Scanning dialog already closed: $scanningDialogAlreadyClosed');
        ...
        if (!scanningDialogAlreadyClosed) {
          ...
          print('[HANDLE_MATCHED] Scanning dialog already closed - skipping pop');
        }
        
        // Add a longer delay to ensure the scanning dialog is fully closed
        Future.delayed(Duration(milliseconds: 500), () {
          if (mounted) {
            print('[HANDLE_MATCHED] Showing delayed error dialog - widget still mounted');
            _showErrorDialog('Already Completed', '$learnerName has already completed clocking for today.', skipPop: true);
            print('[HANDLE_MATCHED] Error dialog shown - should stay on dashboard');
          } else {
            print('[HANDLE_MATCHED] Widget disposed before showing error dialog - this might indicate crash');
          }
        });
      }
      ...
      try {
        if (Navigator.of(context).canPop()) {
          ...
          print('[HANDLE_MATCHED] Scanning dialog closed successfully during error');
        }
      } catch (navError) {
      ...
      Future.delayed(Duration(milliseconds: 300), () {
        if (mounted) {
          ...
          _showErrorDialog('Processing Error', 'Error handling matched learner: $e', skipPop: true);
        } else {
          print('[HANDLE_MATCHED] Widget disposed during error - cannot show error dialog');
        }
      });
    }
  }
  ...
  Future<void> _performClockIn(String learnerId, String learnerName) async {
    print('[CLOCK_IN] Starting clock-in for learner: $learnerId ($learnerName)');
  ...
  Future<void> _performClockOut(String learnerId, String learnerName, Map<String, dynamic> existingAttendance) async {
    final now = _getCurrentTimeString();
    ...
    try {
      ...
      await DatabaseHelper().updateClocking(existingAttendance['clocking_id'], updatedAttendance);
  ...
  Future<Map<String, String>> _getCurrentLocation() async {
    ...
    print('[DASHBOARD] Geolocator temporarily disabled - using default coordinates');
    return {
      'latitude': '0.0',
      'longitude': '0.0',
      'accuracy': '10.0',
    ...
    try {
      ...
      if (classQuery.isNotEmpty) {
        ...
        if (suggestedClassID != null && suggestedClassID != widget.classID) {
          print('🎯 Found best classID: $suggestedClassID with ${classQuery.first['learner_count']} learners');
          print('🔧 Current classID: ${widget.classID} → Switching to: $suggestedClassID');
  ...
  Future<void> _debugFingerprintDatabase() async {
    try {
      ...
      final learners = snapshot?['learners'] as List<Map<String, dynamic>>? ?? [];
      ...
      for (var learner in learners.take(10)) { // Check first 10 learners
        final learnerId = learner['LearnerID']?.toString();
        if (learnerId == null) continue;
        ...
        if (rawResult.isNotEmpty) {
          ...
          print('    zkteco_left_template: ${rawRow['zkteco_left_template']?.toString().length ?? 'NULL'} chars');
          print('    zkteco_right_template: ${rawRow['zkteco_right_template']?.toString().length ?? 'NULL'} chars');
          print('    futronic_left_template: ${rawRow['futronic_left_template']?.toString().length ?? 'NULL'} chars');
          print('    futronic_right_template: ${rawRow['futronic_right_template']?.toString().length ?? 'NULL'} chars');
          
          // Check if templates contain actual data vs empty strings
          final futronicLeftRaw = rawRow['futronic_left_template']?.toString();
          final futronicRightRaw = rawRow['futronic_right_template']?.toString();
          print('    futronic_left_template actual content: ${futronicLeftRaw?.substring(0, futronicLeftRaw.length > 50 ? 50 : futronicLeftRaw.length) ?? 'NULL'}...');
          print('    futronic_right_template actual content: ${futronicRightRaw?.substring(0, futronicRightRaw.length > 50 ? 50 : futronicRightRaw.length) ?? 'NULL'}...');
        }
        ...
        if (hasLeft || hasRight) {
          ...
          print('    Right: ${hasRight ? "${rightTemplate.length} chars" : "❌"}');
        } else {
      ...
      print('  Total learners with ANY fingerprint data: $totalFingerprintCount');
      ...
      print('  Learners with Futronic templates: ${futronicCount.first['count']}');
      ...
      print('  Percentage with fingerprints: ${learnersChecked > 0 ? ((learnersWithFingerprints / learnersChecked) * 100).toStringAsFixed(1) : 0}%');
      
      // Check if we have fingerprints but no learners (ClassID mismatch)
      print('🔍 Debug check: learnersChecked=$learnersChecked, totalFingerprintCount=$totalFingerprintCount');
      if (learnersChecked == 0 && totalFingerprintCount > 0) {
        ...
        if (classQuery.isNotEmpty) {
          ...
          for (var row in classQuery) {
            print('   ClassID ${row['classID']}: ${row['learner_count']} learners with fingerprints');
          }
          
          final suggestedClassID = classQuery.first['classID']?.toString();
          print('\n💡 SOLUTION: Current classID "${widget.classID}" has no learners.');
          print('💡 Suggested classID "$suggestedClassID" which has ${classQuery.first['learner_count']} learners with fingerprints');
          ...
          if (suggestedClassID != null) {
            ...
            try {
              final learnersFromCorrectClass = await DatabaseHelper().getLearnersWithClockingData(suggestedClassID);
              if (learnersFromCorrectClass.isNotEmpty) {
                print('✅ Found ${learnersFromCorrectClass.length} learners in suggested classID');
                ...
                print('✅ currentLearners updated with ${currentLearners.length} learners from correct classID');
              }
            } catch (e) {
      ...
      print('❌ Debug error: $e');
      _showErrorDialog('Debug Error', 'Error debugging database: $e', skipPop: true);
    }
  }
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Row(
            children: [
              Icon(isUpdate ? Icons.update : Icons.refresh, color: Colors.blue, size: 30),
              SizedBox(width: 12),
  ...
  Future<void> _processFingerprintUpdate(Map<String, dynamic> learner, bool isUpdate) async {
    final learnerId = learner['LearnerID']?.toString();
    ...
    try {
      ...
      final scanner = await _detectScanner();
      if (scanner == 'none') {
        _showErrorDialog('Scanner Not Found', 'No fingerprint scanner detected. Please connect a scanner and try again.');
        return;
      }
  ...
  void _showFingerprintUpdateDialog(Map<String, dynamic> learner, String scanner, bool isUpdate) {
    final learnerName = '${learner['Name']} ${learner['Surname']}';
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Row(
            children: [
              Icon(isUpdate ? Icons.update : Icons.refresh, color: Colors.orange, size: 30),
              SizedBox(width: 12),
              Text(isUpdate ? 'Update Fingerprints' : 'Re-enroll Fingerprints'),
            ],
          ),
  ...
  Future<void> _startFingerprintEnrollmentProcess(Map<String, dynamic> learner, String scanner, bool isUpdate) async {
    final learnerId = learner['LearnerID']?.toString();
    final learnerName = '${learner['Name']} ${learner['Surname']}';
    ...
    try {
      ...
      if (result == true) {
        ...
        _showSuccessDialog(learnerName, isUpdate ? 'Fingerprint Update' : 'Fingerprint Re-enrollment');
      }
    } catch (e) {
      _showErrorDialog('Enrollment Error', 'Error starting fingerprint enrollment: $e');
    }
  }
  ...
  void _showSuccessDialog(String learnerName, String action) {
    print('[SUCCESS_DIALOG] _showSuccessDialog called for $learnerName, action: $action');
            ...
            TextButton.icon(
              onPressed: () {
                ...
                try {
                  ...
                  Navigator.of(context).pop();
                  print('[SUCCESS_DIALOG] Dialog popped, checking mounted state: $mounted');
                  
                  // Reload class data to reflect new attendance, but only if widget is still mounted
                  if (mounted) {
                    print('[SUCCESS_DIALOG] Widget is mounted, scheduling data reload');
                    // Schedule the reload after the dialog is fully dismissed
                    Future.delayed(Duration(milliseconds: 200), () {
                      if (mounted) {
                        print('[SUCCESS_DIALOG] Starting delayed data reload');
                        setState(() {
                          print('[SUCCESS_DIALOG] Inside setState, calling reloadClassData');
                          reloadClassData();
                        });
                        print('[SUCCESS_DIALOG] Delayed reload completed successfully');
                      } else {
                        print('[SUCCESS_DIALOG] Widget disposed during delayed reload, skipping');
                      }
                    });
                  } else {
                    print('[SUCCESS_DIALOG] Widget is not mounted, skipping setState');
                  }
                } catch (e) {
                  print('[SUCCESS_DIALOG] ERROR in OK button handler: $e');
                  print('[SUCCESS_DIALOG] Stack trace: ${StackTrace.current}');
                }
              },
  ...
  void _showErrorDialog(String title, String message, {bool skipPop = false}) {
    print('[ERROR_DIALOG] _showErrorDialog called: $title - $message, skipPop: $skipPop');
            ...
            TextButton.icon(
              onPressed: () {
                ...
                try {
                  ...
                  print('[ERROR_DIALOG] Current navigator can pop: ${Navigator.of(dialogContext).canPop()}');
                  Navigator.of(dialogContext).pop();
                  print('[ERROR_DIALOG] Dialog closed successfully - staying on dashboard');
                  print('[ERROR_DIALOG] Dashboard widget still mounted: $mounted');
                } catch (e) {
          ...
          content: Column(
            ...
            children: [
              Container(
                ...
                child: Column(
                  children: [
                    Icon(Icons.fingerprint_outlined, size: 48, color: Colors.orange),
                    SizedBox(height: 12),
                    Text(
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            icon: Icon(Icons.settings_backup_restore, color: Colors.purple, size: 20),
            tooltip: 'Fix ClassID',
            onPressed: _forceClassIDFix,
          ),
          IconButton(
            icon: Icon(Icons.fingerprint_outlined, color: Colors.blue, size: 20),
      ...
      body: FutureBuilder<Map<String, dynamic>>(
        ...
        builder: (context, snapshot) {
          print("[DASHBOARD] FutureBuilder state - Connection: ${snapshot.connectionState}, hasData: ${snapshot.hasData}, data: ${snapshot.data}");
          ...
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            print("[DASHBOARD] FutureBuilder no data - hasData: ${snapshot.hasData}, data: ${snapshot.data}");
            return Center(
              child: Column(
          ...
          final totalLearners = classInfo['total_learners']?.toString() ?? 'N/A';
  ...
  void _navigateToFacilitatorFingerprints() async {
    try {
      ...
      debugPrint('[DASHBOARD] Total facilitators in database: ${allFacilitators.length}');
      ...
      debugPrint('[DASHBOARD] Facilitators found for classID ${widget.classID}: ${facilitators.length}');
      ...
      if (facilitators.isNotEmpty) {
        ...
        final fullName = '${facilitator['firstName'] ?? ''} ${facilitator['lastName'] ?? ''}'.trim();
        
        debugPrint('[DASHBOARD] Facilitator ID: $facilitatorId, Name: $fullName');
        ...
        if (facilitatorId != null) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Facilitator ID not found. Please contact support.'),
              backgroundColor: Colors.orange,
            ),
          );
        }
      } else {
        debugPrint('[DASHBOARD] No facilitator found for classID: ${widget.classID}');
        
        // Try to find any facilitator and show helpful message
        if (allFacilitators.isNotEmpty) {
          final availableClassIDs = allFacilitators.map((f) => f['classID']).toSet().toList();
          debugPrint('[DASHBOARD] Available classIDs: $availableClassIDs');
          
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('No facilitator found for class ${widget.classID}. Available classes: $availableClassIDs'),
              backgroundColor: Colors.orange,
  ...
  Widget _buildInfoCard(BuildContext context, {required String title, required List<String> details}) {
    return Card(
        ...
        child: Column(
          children: [
            ...
            Row(
              ...
              children: [
                _buildBiometricOption(context, Icons.fingerprint, 'Fingerprint'),
                SizedBox(width: 16),
  ...
  Widget _buildBiometricOption(BuildContext context, IconData icon, String label) {
