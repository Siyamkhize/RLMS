<?php
// UNIFIED ARPL PDF GENERATOR - Single File Solution
// Works from /web/ directory
// Generates 30+ page ARPL portfolio

session_start();
// Connection path from /web/ directory: go up one level to root
...
// ── Extract Parameters ────────────────────────────────────────
// Support parameters in different formats
$classID   = isset($_GET['classID']) ? (int)$_GET['classID'] : 0;
$learnerID = isset($_GET['learnerID']) ? (int)$_GET['learnerID'] : 0;
$ofoNumber = isset($_GET['ofoNumber']) ? trim($_GET['ofoNumber']) : 
             (isset($_GET['ofo_code']) ? trim($_GET['ofo_code']) : '642601');
...
if (!$classID || !$learnerID) {
    die(json_encode(['status'=>'error','message'=>'Missing classID or learnerID']));
}
