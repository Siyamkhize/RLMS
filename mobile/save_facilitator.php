...
$f_IDNumber = isset($data['f_IDNumber']) ? $conn->real_escape_string($data['f_IDNumber']) : null;
...
if ($exists) {
    ...
    if ($f_IDNumber !== null && $f_IDNumber !== '') {
        $updateFields[] = "f_IDNumber = ?";
        $updateValues[] = $f_IDNumber;
        $types .= "s";
    }
    ...
    // INSERT operation
    $sql = "INSERT INTO facilitator (classID, phoneNumber, f_IDNumber, assessorNo, assessorExpiryDate, f_signature)
            VALUES (?, ?, ?, ?, ?, ?)";
    ...
    $stmt->bind_param("ssssss", $classID, $phoneNumber, $f_IDNumber, $assessorNo, $assessorExpiryDate, $signaturePath);
