<?php
/**
 * Add Supplemental Learners - FAST VERSION
 * Optimized for speed with 62 classes
 */
...
try {
    ...
    if ($method === 'POST') {
        ...
        // Step 2: Get moderator's classes (FAST - single query)
        ...
        if (empty($classIds)) {
            throw new Exception("No classes allocated to moderator (or only testing class 74)");
        }
        
        // Step 3: Get available learners using LEFT JOIN (MUCH FASTER than NOT IN)
        ...
        if (empty($learners)) {
            ...
            echo json_encode([
                'status' => 'success',
                'message' => 'No additional learners available',
                'data' => [
                    'current_count' => $currentCount,
                    'target_count' => $targetCount,
        ...
        // Step 4: Add supplemental assignments (BATCH INSERT for speed)
