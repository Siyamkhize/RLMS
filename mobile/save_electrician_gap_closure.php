<?php
/**
 * Save Electrician Gap Closure Recommendations
 * Saves selected unit standards, recommendation status, and candidate marks/score.
 * Electrician uses qualification 91761 + occupational_unit_standards table (different from brick/plumber).
 * Auto-adds CandidateScore column if missing.
 *
 * Endpoint: POST mobile/save_electrician_gap_closure.php
 *
 * Request:
 * {
 *   "learnerID": 20286,
...
ensure_column($conn, 'arplelectrician_access_recommendation', 'CandidateScore',
...
try {
    ...
    // ELECTRICIAN = occupational_unit_standards table. Robust column detection.
    $TABLE_NAME = 'occupational_unit_standards';
    $DEFAULT_QUAL = 91761;
    $checkColumn = $conn->query("SHOW COLUMNS FROM $TABLE_NAME");
    ...
    try {
        $delete_stmt = $conn->prepare("DELETE FROM arplelectrician_access_recommendation WHERE LearnerID = ?");
        ...
        $delete_gap_stmt = $conn->prepare("
            DELETE FROM arplelectrician_gap_unit_standards WHERE learner_id = ?
        ");
        ...
        if ($overall_result_status === 'Recommended for Gap Closure' && !empty($selected_unit_standards)) {
            ...
            $gap_stmt = $conn->prepare("
                INSERT INTO arplelectrician_gap_unit_standards
