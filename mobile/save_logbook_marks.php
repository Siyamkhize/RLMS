...
$marks = isset($input['marks']) ? $input['marks'] : [];

if (empty($learner_id) || empty($assessor_id) || empty($assessment_date) || empty($marks)) {
    echo json_encode([
        'status' => 'error',
...
try {
    ...
    // $marks is a map of unit_standard_id => mark_value
    foreach ($marks as $unit_standard_id => $mark_value) {
