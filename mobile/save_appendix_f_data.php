...
try {
    ...
    if (!empty($knowledge) && is_array($knowledge)) {
        ...
        $deleteResult = $conn->query("DELETE FROM arpl_appendix_f_knowledge WHERE learnerID = $learnerID AND ofoNumber = '$ofoNumber'");
        if (!$deleteResult) {
            error_log("❌ Appendix F Knowledge DELETE failed: " . $conn->error);
            throw new Exception("Failed to clear previous knowledge data: " . $conn->error);
        } else {
            error_log("✅ Appendix F Knowledge DELETE succeeded (affected rows: " . $conn->affected_rows . ")");
        }
    ...
    if (!empty($practical) && is_array($practical)) {
        ...
        $deleteResult = $conn->query("DELETE FROM arpl_appendix_f_practical_tasks WHERE learnerID = $learnerID AND ofoNumber = '$ofoNumber'");
        if (!$deleteResult) {
            error_log("❌ Appendix F Practical DELETE failed: " . $conn->error);
            throw new Exception("Failed to clear previous practical data: " . $conn->error);
        } else {
            error_log("✅ Appendix F Practical DELETE succeeded (affected rows: " . $conn->affected_rows . ")");
        }
