...
// Check database for valid 24-hour access
$otp_verified = false;
$id_number = '';

// First, check if we have an ID number (from session or we'll ask for it)
if (isset($_SESSION['agreement_portal_id'])) {
    $id_number = $_SESSION['agreement_portal_id'];
    
    // Check database for active access
    require_once '../connection.php';
    
    $access_check_stmt = $conn->prepare("
        SELECT 
            o.id,
            o.access_expires_at,
            l.LearnerID,
    ...
    $access_check_stmt->bind_param('s', $id_number);
    $access_check_stmt->execute();
    $access_result = $access_check_stmt->get_result();
    
    if ($access_result->num_rows > 0) {
        // Valid 24-hour access found in database
        $access_data = $access_result->fetch_assoc();
        $otp_verified = true;
        
        // Update session with current access expiry
        ...
        // No valid access found, clear session
        session_destroy();
        session_start();
    }
    
    $access_check_stmt->close();
}

// Handle logout
if (isset($_GET['logout'])) {
    // Clear session
    $logout_id = $_SESSION['agreement_portal_id'] ?? '';
    session_destroy();
    
    // Optionally: Clear database access (uncomment if you want logout to revoke 24-hour access)
    // if (!empty($logout_id)) {
    //     require_once '../connection.php';
    //     $logout_stmt = $conn->prepare("
    //         UPDATE seda_otp_verification 
    //         SET access_expires_at = NOW() 
    //         WHERE learner_id IN (SELECT LearnerID FROM learnerdetails WHERE IDNumber = ?)
    //         AND access_expires_at > NOW()
    //     ");
    //     $logout_stmt->bind_param('s', $logout_id);
    //     $logout_stmt->execute();
    // }
    
    header('Location: learner_agreement_portal.php');
    exit;
}
...
<body>
    ...
    <script>
        ...
        // Check for existing access when ID is entered
        document.getElementById('id_number')?.addEventListener('blur', async function(e) {
            const idNumber = this.value;
            if (idNumber.length === 13) {
                await checkExistingAccess(idNumber);
            }
        });

        // Check for existing access
        async function checkExistingAccess(idNumber) {
            try {
                const response = await fetch('check_existing_access.php', {
                    method: 'POST',
                ...
                if (data.success && data.has_access) {
                    // User already has valid access - set session and redirect
                    if (confirm(data.message + ' Click OK to access your agreements now.')) {
                        // Call resume access to set session
                        const resumeResponse = await fetch('resume_access.php', {
                            method: 'POST',
                            headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                            body: `id_number=${encodeURIComponent(idNumber)}`
                        });
