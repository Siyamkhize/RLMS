...
try {
    ...
    foreach ($input as $item) {
        ...
        if ($existingLearner) {
            ...
            // CRITICAL: Preserve existing signatures/profile_image/initials if incoming data is empty
            // Fetch existing values for protected fields
            $protectStmt = $conn->prepare("SELECT profile_image, signature, witness_signature, learner_initials, witness_initials FROM learnerdetails WHERE LearnerID = ?");
            $protectStmt->bind_param("i", $serverLearnerId);
            $protectStmt->execute();
            $existingData = $protectStmt->get_result()->fetch_assoc();
            $protectStmt->close();
            
            // Preserve existing values if incoming values are empty/null
            if (empty($fields['profile_image']) && !empty($existingData['profile_image'])) {
                $fields['profile_image'] = $existingData['profile_image'];
            }
            if (empty($fields['signature']) && !empty($existingData['signature'])) {
                $fields['signature'] = $existingData['signature'];
            }
            if (empty($fields['witness_signature']) && !empty($existingData['witness_signature'])) {
                $fields['witness_signature'] = $existingData['witness_signature'];
            }
            if (empty($fields['learner_initials']) && !empty($existingData['learner_initials'])) {
                $fields['learner_initials'] = $existingData['learner_initials'];
            }
            if (empty($fields['witness_initials']) && !empty($existingData['witness_initials'])) {
                $fields['witness_initials'] = $existingData['witness_initials'];
            }
