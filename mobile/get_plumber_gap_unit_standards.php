<?php
/**
 * Get Plumber Gap Closure Unit Standards
 * Fetches all unit standards for qualification 65409 (shared with Bricklayer)
 * Used when assessor selects "Recommended for Gap Closure" in Appendix H
 *
 * Endpoint: POST mobile/get_plumber_gap_unit_standards.php
 *
 * Request:
 * {
 *   "learnerID": 20286,
...
try {
    ...
    // ══════════════════════════════════════════════════════════
    // GET ALL UNIT STANDARDS FOR QUALIFICATION 65409 (shared brick/plumber)
    // ══════════════════════════════════════════════════════════
    $targetQual = $qualification_id;
    $stmt = $conn->prepare("
        SELECT
    ...
    $stmt = $conn->prepare("
        ...
        FROM arplplumber_gap_unit_standards
        WHERE learner_id = ?
    ");
    ...
    echo json_encode([
        'status' => 'success',
        'learnerID' => $learnerID,
        'qualification_id' => $targetQual,
        'qualification_requested' => $qualification_id,
        'trade' => 'plumber',
        'ofo_code' => '642601',
