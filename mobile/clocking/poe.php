...
if (!isset($_POST['learnerID']) || !is_numeric($_POST['learnerID'])) {
    echo json_encode([
        'error' => 'Invalid or missing learnerID.',
        'debug' => ['post_params' => $_POST]
    ]);
    exit;
}

$learnerID = (int)$_POST['learnerID'];
        ...
        if ($row['unit_standard_id'] && (string)$row['unit_standard_id'] === (string)$unitStandardId) {
            ...
            if (in_array($assessmentType, ['formative', 'summative'])) {
                ...
                if ($assessmentType == 'formative') {
                    $data['pathways'][$pathwayName]['qualifications'][$qualificationName]['unitstandards'][$unitStandardName]['formative'][] = $assessment;
                } elseif ($assessmentType == 'summative') {
                    $data['pathways'][$pathwayName]['qualifications'][$qualificationName]['unitstandards'][$unitStandardName]['summative'][] = $assessment;
                    if ($questionType == 'Practical') {
                        $data['pathways'][$pathwayName]['qualifications'][$qualificationName]['unitstandards'][$unitStandardName]['logbook'][] = $assessment;
                    }
                }
            } else {
