...
$checkStmt = $conn->prepare("SELECT id FROM arpl_theory WHERE learnerID = ? AND ofo_number = ? AND paper_number = ?");
if (!$checkStmt) {
...
if ($checkResult->num_rows > 0) {
    error_log('Duplicate theory upload found');
    sendResponse('error', 'This theory paper has already been uploaded');
}
$checkStmt->close();
...
foreach ($_FILES['files']['name'] as $key => $name) {
    ...
    error_log("Processing THEORY file $key: $name, Size: $fileSize bytes");
    ...
    // Generate filename: All_Questions_[Paper_Title]_[OFO]_theory.pdf
    ...
    $fileName = 'All_Questions_' . $sanitizedPaper . '_' . $sanitizedOFO . '_theory.' . $extension;
    ...
    if (move_uploaded_file($tmpPath, $destinationPath)) {
        ...
        error_log("THEORY PDF uploaded: $name -> $destinationPath");
    } else {
...
try {
    ...
    // Insert into arpl_theory table
    $stmt = $conn->prepare('INSERT INTO arpl_theory (learnerID, ofo_number, paper_title, paper_number, question_count, combined_pdf_path, file_name, upload_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
    if (!$stmt) throw new Exception('Database prepare error: ' . $conn->error);
    
    $uploadStatus = 'uploaded';
    $stmt->bind_param('sssiisss', $learnerID, $ofoNumber, $paperTitle, $paperNumber, $questionCount, $filePath, $fileName, $uploadStatus);
    
    if (!$stmt->execute()) {
        throw new Exception('Failed to insert theory upload: ' . $stmt->error);
    }
    ...
    $response = [
        'status' => 'success',
        'message' => 'Theory PDF uploaded successfully',
    ...
    error_log("ARPL THEORY upload completed for learnerID=$learnerID, paper=$paperNumber, file=$filePath");
