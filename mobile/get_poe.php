...
if ($_SERVER['REQUEST_METHOD'] == 'POST' || $_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    $query = "
        ...
        LEFT JOIN poe p ON p.learnerID = ? 
            ...
            AND (
                -- Case 1: POE has NO unit standard IDs anywhere (legacy files), match freely
                (p.filePath NOT REGEXP '_[0-9]{4,10}_' AND p.exercise NOT REGEXP '[0-9]{4,10}')
                -- OR
                -- Case 2: POE has AT LEAST ONE unit standard ID, and it MATCHES the assessment's unit standard ID
                OR (
                    (p.filePath LIKE CONCAT('%_', a.unit_standard_id, '_%') OR p.exercise LIKE CONCAT('%', a.unit_standard_id, '%'))
                )
            )
        LEFT JOIN marks m ON m.learnerID = ? 
            ...
            AND (
                -- Case 1: Mark has NO unit standard IDs anywhere (legacy entries), match freely
                m.exercise NOT REGEXP '[0-9]{4,10}'
                -- OR
                -- Case 2: Mark has AT LEAST ONE unit standard ID, and it MATCHES the assessment's unit standard ID
                OR m.exercise LIKE CONCAT('%', a.unit_standard_id, '%')
            )
