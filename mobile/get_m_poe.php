...
if ($_SERVER['REQUEST_METHOD'] == 'POST' || $_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    // ENHANCED: Added original_assessment_type and poe_type for debugging swap issue
    $query = "
        SELECT DISTINCT
            ...
            a.assessment_type AS original_assessment_type,
            a.question_number,
            a.specific_outcome,
        ...
        GROUP BY
            ...
            pathway_name, qualification_name, unit_standards
        ORDER BY
    ...
    // Debug: Log raw query results with learnerID context
    $rawData = [];
    while ($row = $result->fetch_assoc()) {
        $rawData[] = $row;
    }
    file_put_contents('debug.log', "Raw query results for learnerID $learnerID: " . print_r($rawData, true) . "\n", FILE_APPEND);
    if (!$result->num_rows) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        foreach ($uniqueUnitStandards as $unitStandard) {
            ...
            if ($row['unit_standard_id'] && (string)$row['unit_standard_id'] === (string)$unitStandardId) {
                // ENHANCED: Enhanced type processing with swap detection/logging and temporary fix
                $originalType = $row['original_assessment_type'] ?? 'N/A';
                $poeType = $row['poe_type'] ?? 'N/A';
                $assessmentType = strtolower(trim($row['assessment_type'] ?? 'unknown'));
                
                // TEMPORARY FIX: Force-swap formative and summative (uncommented for immediate relief; comment out after DB fix)
                if ($assessmentType === 'formative') {
                    $assessmentType = 'summative';
                    file_put_contents('debug.log', "SWAPPED: formative -> summative for exercise={$row['exercise']} (poe_exercise={$row['poe_exercise']})\n", FILE_APPEND);
                } elseif ($assessmentType === 'summative') {
                    $assessmentType = 'formative';
                    file_put_contents('debug.log', "SWAPPED: summative -> formative for exercise={$row['exercise']} (poe_exercise={$row['poe_exercise']})\n", FILE_APPEND);
                }
                
                // Enhanced logging for debugging
                file_put_contents('debug.log', "Row debug - exercise={$row['exercise']}, poe_exercise={$row['poe_exercise']}, original_type=$originalType, assessment_type={$row['assessment_type']}, poe_type=$poeType, processed_type=$assessmentType, unit_id={$row['unit_standard_id']}, filePath={$row['filePath']}\n", FILE_APPEND);
                ...
                if (in_array($assessmentType, ['formative', 'summative', 'logbook'])) {
                    ...
                    file_put_contents('debug.log', "Assessment data added to $assessmentType: " . print_r($assessment, true) . "\n", FILE_APPEND);
