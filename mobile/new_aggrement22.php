    ...
    foreach ($forms_list as $form_name) {
        ...
        try {
            ...
            if ($form_template_path && file_exists($form_template_path)) {
                ...
                // Replace placeholders for DoL EEA1 Form

                if ($form_name === 'DoL EEA1 Form') {

                    // Set specific fields required for DoL EEA1 Form
                    ...
                    $disability = strtolower(trim($learner_data['Disability'] ?? ''));
                    ...
                    // Add learner signature image for DoL EEA1 Form
                    ...
                    if ($learner_signature_path) {
                        ...
                        log_message("DoL EEA1 Form - Learner signature image added: $learner_signature_path");

                    } else {

                        $template->setValue('learner_signature', 'N/A');

                        log_message("DoL EEA1 Form - No learner signature found");
        ...
        if (count($generatedFiles) === 1 && !$forceZip) {
            ...
            try {
                ...
                if (file_exists($zipFileName)) {
                    ...
                    foreach ($docxFilesToCleanup as $docxFile) {

                        @unlink($docxFile);

                        log_message("Cleaned up DOCX file: $docxFile");
                ...
                if (file_exists($file)) {
                    ...
                    if ($fileExtension === 'docx') {
                        ...
                        if ($pdfPath && file_exists($pdfPath)) {
                            ...
                            $relativePath = str_replace($output_dir, '', $pdfPath);

                            $zip->addFile($pdfPath, $relativePath);

                            

                            // Clean up DOCX after adding PDF to ZIP

                            @unlink($file);

                            log_message("Cleaned up DOCX file: $file");
