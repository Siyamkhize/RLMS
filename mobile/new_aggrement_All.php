...
$session_project_id = isset($_SESSION['project_id']) ? (int)$_SESSION['project_id'] : null;
...
// Handle Debug Mode - Find missing learners
if (isset($_GET['debug_missing']) && $_GET['debug_missing'] == '1') {
    $skip_attendance_filter = isset($_GET['skip_filter']) && $_GET['skip_filter'] == '1';
    echo "<h2>Debug: Finding Missing Learners</h2>";
    
    // Query from overall.php (using broader date range to capture more attendance)
    $startDate_debug = date('Y-m-d', strtotime('-30 days')); // Broader range to capture more attendance
    $endDate_debug = date('Y-m-d'); // Current date
    
    echo "<p><strong>Debug Info:</strong></p>";
    echo "<p>Overall.php date range: {$startDate_debug} to {$endDate_debug}</p>";
    ...
    $overall_sql = "
        ...
        LEFT JOIN (
            ...
            WHERE DATE(clock_date) BETWEEN '{$startDate_debug}' AND '{$endDate_debug}'
        ) lc ON ld.LearnerID = lc.LearnerID
        WHERE (ld.activity_statu = '' OR ld.activity_statu IS NULL)
        ORDER BY ld.LearnerID
    ";
    
    // Query from stipendcsv.php (matching main CSV query structure)
    $year_debug = date('Y');
    $month_debug = date('n');
    $startDate_csv = sprintf('%04d-%02d-01', $year_debug, $month_debug);
    $endDate_csv = date('Y-m-t', mktime(0, 0, 0, $month_debug, 1, $year_debug));
    
    echo "<p>Stipendcsv.php date range: {$startDate_csv} to {$endDate_csv}</p>";
    ...
    $stipend_sql = "
        ...
        LEFT JOIN sdp ON sdp.sdp_id = s.sdp_id
        LEFT JOIN bankdetails bd ON bd.learnerID = ld.learnerID
        LEFT JOIN (
            SELECT DISTINCT learner_id, status
            FROM sick_note 
            WHERE DATE(date_from) BETWEEN '{$startDate_csv}' AND '{$endDate_csv}'
            AND UPPER(status) = 'APPROVED'
        ) sn ON ld.learnerID = sn.learner_id
        LEFT JOIN (
            SELECT 
                lc.learnerID,
                COUNT(DISTINCT DATE(lc.clock_date)) as days_present
            FROM learner_clocking lc
            WHERE DATE(lc.clock_date) BETWEEN '{$startDate_csv}' AND '{$endDate_csv}'
            GROUP BY lc.learnerID
        ) attendance ON attendance.learnerID = ld.learnerID
        LEFT JOIN (
            SELECT 
                sn.learner_id,
                COUNT(DISTINCT sn.date_from) as approved_sick_days
            FROM sick_note sn
            WHERE DATE(sn.date_from) BETWEEN '{$startDate_csv}' AND '{$endDate_csv}'
            AND UPPER(sn.status) = 'APPROVED'
            AND DAYOFWEEK(sn.date_from) NOT IN (1, 7)
            GROUP BY sn.learner_id
        ) sick_days ON sick_days.learner_id = ld.learnerID
        WHERE (ld.activity_statu = '' OR ld.activity_statu IS NULL)
        ORDER BY ld.learnerID
    ";
    
    $overall_result = $conn->query($overall_sql);
    $stipend_result = $conn->query($stipend_sql);
    
    if (!$overall_result || !$stipend_result) {
        die("Query error: " . $conn->error);
    }
    
    // Get all learner IDs from both queries
    $overall_learners = [];
    $stipend_learners = [];
    
    while ($row = $overall_result->fetch_assoc()) {
        $overall_learners[$row['LearnerID']] = $row;
    }
    
    while ($row = $stipend_result->fetch_assoc()) {
        $stipend_learners[$row['learnerID']] = $row;
    }
    
    // Find missing learners
    $missing_learners = array_diff_key($overall_learners, $stipend_learners);
    
    // Apply attendance filtering to overall.php results (simulate the 0% attendance filter)
    if ($skip_attendance_filter) {
        $filtered_overall_learners = $overall_learners;
        $zero_attendance_count = 0;
        echo "<p><strong>SKIPPING ATTENDANCE FILTER</strong> - showing raw comparison</p>";
    } else {
        $filtered_overall_learners = [];
        $zero_attendance_count = 0;
        
        foreach ($overall_learners as $learner) {
            // Simulate attendance calculation - check if learner has any attendance in the date range
            $attendance_check = "SELECT COUNT(*) as attendance_count FROM learner_clocking WHERE learnerID = {$learner['LearnerID']} AND DATE(clock_date) BETWEEN '{$startDate_debug}' AND '{$endDate_debug}'";
            $attendance_result = $conn->query($attendance_check);
            $attendance_row = $attendance_result->fetch_assoc();
            $has_attendance = $attendance_row['attendance_count'] > 0;
            
            if ($has_attendance) {
                $filtered_overall_learners[$learner['LearnerID']] = $learner;
            } else {
                $zero_attendance_count++;
            }
        }
        
        echo "<p>Learners with 0% attendance (excluded): {$zero_attendance_count}</p>";
    }
    
    // Recalculate missing learners with filtered overall results
    $missing_learners = array_diff_key($filtered_overall_learners, $stipend_learners);
    
    echo "<h3>Results:</h3>";
    echo "<p><strong>Overall.php learners (raw):</strong> " . count($overall_learners) . "</p>";
    echo "<p><strong>Overall.php learners (filtered - excluding 0% attendance):</strong> " . count($filtered_overall_learners) . "</p>";
    echo "<p><strong>Stipendcsv.php learners:</strong> " . count($stipend_learners) . "</p>";
    echo "<p><strong>Missing learners:</strong> " . count($missing_learners) . "</p>";
    
    if (count($missing_learners) > 0) {
        echo "<h3>Missing Learners Details:</h3>";
        echo "<table border='1' cellpadding='5' cellspacing='0'>";
        echo "<tr><th>LearnerID</th><th>Name</th><th>Surname</th><th>IDNumber</th><th>Site</th><th>District</th><th>Project ID</th><th>SDP ID</th></tr>";
        
        foreach ($missing_learners as $learner) {
            echo "<tr>";
            echo "<td>" . htmlspecialchars($learner['LearnerID']) . "</td>";
            echo "<td>" . htmlspecialchars($learner['Name']) . "</td>";
            echo "<td>" . htmlspecialchars($learner['Surname']) . "</td>";
            echo "<td>" . htmlspecialchars($learner['IDNumber']) . "</td>";
            echo "<td>" . htmlspecialchars($learner['siteName']) . "</td>";
            echo "<td>" . htmlspecialchars($learner['District']) . "</td>";
            echo "<td>" . htmlspecialchars($learner['project_id']) . "</td>";
            echo "<td>" . htmlspecialchars($learner['sdp_id']) . "</td>";
            echo "</tr>";
        }
        echo "</table>";
        
        // Check what's missing for these learners
        if (count($missing_learners) > 0) {
            $missing_ids = array_keys($missing_learners);
            $ids_string = implode(',', $missing_ids);
            
            echo "<h3>Debugging Missing Learners:</h3>";
            
            // Check if they have bank details
            $bank_check = "SELECT learnerID, BankName, BankAccount FROM bankdetails WHERE learnerID IN ($ids_string)";
            $bank_result = $conn->query($bank_check);
            echo "<h4>Bank Details:</h4>";
            if ($bank_result && $bank_result->num_rows > 0) {
                while ($row = $bank_result->fetch_assoc()) {
                    echo "<p>Learner {$row['learnerID']}: {$row['BankName']} - {$row['BankAccount']}</p>";
                }
            } else {
                echo "<p>No bank details found for missing learners</p>";
            }
            
            // Check if they have document records
            $doc_check = "SELECT learner_id, documentName, status FROM learner_document WHERE learner_id IN ($ids_string) AND documentName = 'Learner Agreement'";
            $doc_result = $conn->query($doc_check);
            echo "<h4>Document Records:</h4>";
            if ($doc_result && $doc_result->num_rows > 0) {
                while ($row = $doc_result->fetch_assoc()) {
                    echo "<p>Learner {$row['learner_id']}: {$row['documentName']} - {$row['status']}</p>";
                }
            } else {
                echo "<p>No document records found for missing learners</p>";
            }
        }
    }
...
// Handle Payment CSV Download
if (isset($_GET['download_payment_csv']) && $_GET['download_payment_csv'] == '1') {
    // Check for debug mode
    $debug_csv = isset($_GET['debug_csv']) && $_GET['debug_csv'] == '1';
    
    // Check if required parameters are present
    if (!isset($_GET['project_id']) || !isset($_GET['year']) || !isset($_GET['month'])) {
        die("Error: Missing required parameters for CSV download.");
    }

    // Retrieve and validate parameters
    $project_id = filter_input(INPUT_GET, 'project_id', FILTER_VALIDATE_INT);
    if (!$project_id) { $project_id = $session_project_id ?: 76; }
    $year = filter_input(INPUT_GET, 'year', FILTER_VALIDATE_INT);
    $month = filter_input(INPUT_GET, 'month', FILTER_VALIDATE_INT);
    
    // Get DR account details (optional)
    $dr_account_name = filter_input(INPUT_GET, 'dr_account_name', FILTER_SANITIZE_FULL_SPECIAL_CHARS) ?: '';
    $dr_account_number = filter_input(INPUT_GET, 'dr_account_number', FILTER_SANITIZE_FULL_SPECIAL_CHARS) ?: '';
    $dr_branch_number = filter_input(INPUT_GET, 'dr_branch_number', FILTER_SANITIZE_FULL_SPECIAL_CHARS) ?: '';

    if ($project_id === false || $year === false || $month === false || $month < 1 || $month > 12) {
        die("Error: Invalid project_id, year, or month.");
    }
    ...
    $project_query = "SELECT Project_name FROM project WHERE project_id = ?";
    ...
    // Get document status filter and check for sick notes
    $documentStatusFilter = isset($_GET['document_status']) ? $_GET['document_status'] : '';
    
    // Check for pending sick notes in the selected date range
    $check_pending_sick_notes_sql = "
        SELECT 1
        FROM sick_note sn
        JOIN learnerdetails ld ON sn.learner_id = ld.learnerID
        WHERE UPPER(sn.status) = 'PENDING'
        AND DATE(sn.date_from) BETWEEN ? AND ?
        AND (ld.activity_statu = '' OR ld.activity_statu IS NULL)
        LIMIT 1
    ";
    
    $stmt_check = $conn->prepare($check_pending_sick_notes_sql);
    $stmt_check->bind_param("ss", $startDate, $endDate);
    $stmt_check->execute();
    $has_pending_sick_notes = $stmt_check->get_result()->num_rows > 0;
    $stmt_check->close();
    
    // If not filtering by "Verified" and there are pending sick notes, disable download
    if ($documentStatusFilter !== 'Verified' && $has_pending_sick_notes) {
        die("Error: Cannot generate payment CSV. There are learners with pending sick notes for the selected period. Learners with pending sick notes are excluded from payment calculations (treated as N/A/not verified). Please filter by 'Verified' status only or resolve pending sick notes first.");
    }

    // SQL query for payment data with updated sick notes logic - using same base structure as overall.php
    $sql = "
        SELECT
            COALESCE(s.project_id, 0) AS project_id,
            COALESCE(p.project_name, 'N/A') AS project_name,
            COALESCE(sdp.sdp_name, 'N/A') AS sdp_name,
            ld.learnerID,
            ld.IDNumber,
            ld.Surname,
            ld.Name,
            CONCAT(ld.Surname, ' ', ld.Name) AS fullname,
            COALESCE(bd.BankName, 'N/A') AS BankName,
            COALESCE(bd.BankAccount, 'N/A') AS BankAccount,
            COALESCE(bd.bankCode, 'N/A') AS bankCode,
            ? AS year,
            ? AS month,
            COALESCE(attendance.days_present, 0) AS days_attended_in_month,
            COALESCE(sick_days.approved_sick_days, 0) AS approved_sick_days,
            COALESCE(attendance.days_present, 0) + COALESCE(sick_days.approved_sick_days, 0) AS total_days_affected,
            COALESCE(working_days.total_days, 0) AS total_working_days_in_month,
            COALESCE(working_days.total_days, 0) - (COALESCE(attendance.days_present, 0) + COALESCE(sick_days.approved_sick_days, 0)) AS days_absent,
            COALESCE(SUBSTRING_INDEX(p.pathway_stipend_types, ',', 1), 'N/A') AS stipend_type,
            COALESCE(CAST(SUBSTRING_INDEX(p.pathway_stipends, ',', 1) AS DECIMAL(10,2)), 0) AS monthly_stipend,
            CASE 
                WHEN p.pathway_stipend_types IS NULL OR p.pathway_stipends IS NULL THEN 0
                WHEN SUBSTRING_INDEX(p.pathway_stipend_types, ',', 1) = 'fixed' THEN
                    ROUND(
                        COALESCE(CAST(SUBSTRING_INDEX(p.pathway_stipends, ',', 1) AS DECIMAL(10,2)), 0) / 
                        COALESCE(working_days.total_days, 1) * 
                        (COALESCE(attendance.days_present, 0) + COALESCE(sick_days.approved_sick_days, 0)), 
                        2
                    )
                WHEN SUBSTRING_INDEX(p.pathway_stipend_types, ',', 1) = 'dynamic' THEN
                    ROUND(
                        COALESCE(CAST(SUBSTRING_INDEX(p.pathway_stipends, ',', 1) AS DECIMAL(10,2)), 0) * 
                        (COALESCE(attendance.days_present, 0) + COALESCE(sick_days.approved_sick_days, 0)),
                        2
            ...
            CASE 
                WHEN p.pathway_stipends IS NULL THEN 0
                WHEN ROUND(((COALESCE(attendance.days_present, 0) + COALESCE(sick_days.approved_sick_days, 0)) / COALESCE(working_days.total_days, 1)) * 100, 1) = 100 THEN
                    COALESCE(CAST(SUBSTRING_INDEX(p.pathway_stipends, ',', 1) AS DECIMAL(10,2)), 0)
                ELSE
                    ROUND(
                        (COALESCE(attendance.days_present, 0) + COALESCE(sick_days.approved_sick_days, 0)) * 
                        (COALESCE(CAST(SUBSTRING_INDEX(p.pathway_stipends, ',', 1) AS DECIMAL(10,2)), 0) / COALESCE(working_days.total_days, 1)), 
                        2
                    )
            END AS amount_due,
            COALESCE(all_docs.learner_agreement_status, 'Missing') as learner_agreement_status
        FROM sites s
        JOIN class c ON s.siteID = c.siteID
        JOIN learnerdetails ld ON c.classID = ld.classID
        LEFT JOIN project p ON p.project_id = s.project_id
        ...
        LEFT JOIN (
            SELECT 
                ld.learner_id,
                MAX(CASE WHEN ld.documentName = 'Learner Agreement' THEN ld.status END) as learner_agreement_status
            FROM learner_document ld
            WHERE ld.documentName = 'Learner Agreement'
            GROUP BY ld.learner_id
        ) all_docs ON ld.learnerID = all_docs.learner_id
        LEFT JOIN (
            SELECT DISTINCT learner_id, status
            FROM sick_note 
            WHERE DATE(date_from) BETWEEN ? AND ?
            AND UPPER(status) = 'APPROVED'
        ) sn ON ld.learnerID = sn.learner_id
        LEFT JOIN (
            SELECT 
                ...
                COUNT(DISTINCT DATE(lc.clock_date)) as days_present
            FROM learner_clocking lc
            WHERE DATE(lc.clock_date) BETWEEN ? AND ?
            GROUP BY lc.learnerID
        ) attendance ON attendance.learnerID = ld.learnerID
        LEFT JOIN (
            SELECT 
                sn.learner_id,
                COUNT(DISTINCT DATE(sn.date_from)) as approved_sick_days
            FROM sick_note sn
            WHERE DATE(sn.date_from) BETWEEN ? AND ?
            AND UPPER(sn.status) = 'APPROVED'
            GROUP BY sn.learner_id
        ) sick_days ON sick_days.learner_id = ld.learnerID
        LEFT JOIN (
            SELECT 
                ? as year,
                ? as month,
                (DATEDIFF(LAST_DAY(CONCAT(?, '-', LPAD(?, 2, '0'), '-01')), CONCAT(?, '-', LPAD(?, 2, '0'), '-01')) + 1) -
                (
                    SELECT COUNT(*)
                    FROM (
                        SELECT DATE(CONCAT(?, '-', LPAD(?, 2, '0'), '-', LPAD(day_num, 2, '0'))) as check_date
                        FROM (
                        ...
                        WHERE day_num <= DAY(LAST_DAY(CONCAT(?, '-', LPAD(?, 2, '0'), '-01')))
                    ) all_days
                    WHERE DAYOFWEEK(check_date) IN (1, 7)
                ) as total_days
        ) working_days
        WHERE (ld.activity_statu = '' OR ld.activity_statu IS NULL)
    ...
    $params = [
        $year, $month,           // SELECT parameters
        $startDate, $endDate,    // sick_note subquery (date range)
        $startDate, $endDate,    // attendance subquery (date range)
        $startDate, $endDate,    // sick_days subquery (date range)
        $year, $month,           // working_days subquery year/month
        $year, $month,           // working_days CONCAT year/month
        $year, $month,           // working_days CONCAT year/month (second occurrence)
        $year, $month,           // working_days inner query year/month
        $year, $month            // working_days LAST_DAY year/month
    ];

    // Debug: Test if we have any document data at all
    $test_query = "SELECT COUNT(*) as doc_count FROM learner_document WHERE documentName = 'Learner Agreement'";
    $test_result = $conn->query($test_query);
    if ($test_result) {
        $test_row = $test_result->fetch_assoc();
        error_log("DEBUG: Found " . $test_row['doc_count'] . " total documents in learner_document table");
    }
    
    // Debug: Test JOIN between learnerdetails and learner_document
    $join_test_query = "
        SELECT COUNT(*) as join_count 
        FROM learnerdetails ld 
        LEFT JOIN learner_document ldoc ON ld.learnerID = ldoc.learner_id 
        WHERE ldoc.documentName = 'Learner Agreement'
    ";
    $join_test_result = $conn->query($join_test_query);
    if ($join_test_result) {
        $join_test_row = $join_test_result->fetch_assoc();
        error_log("DEBUG: Found " . $join_test_row['join_count'] . " records when joining learnerdetails with learner_document");
    }
    
    // Debug: Check sample learner_document data
    $sample_query = "SELECT learner_id, documentName, status FROM learner_document WHERE documentName = 'Learner Agreement' LIMIT 3";
    $sample_result = $conn->query($sample_query);
    if ($sample_result) {
        while ($sample_row = $sample_result->fetch_assoc()) {
            error_log("DEBUG Sample: learner_id=" . $sample_row['learner_id'] . ", documentName=" . $sample_row['documentName'] . ", status=" . $sample_row['status']);
        }
    }
    
    // Debug: Test our specific document subquery
    $doc_subquery = "
        SELECT 
            learner_id,
            SUBSTRING_INDEX(GROUP_CONCAT(CASE WHEN documentName = 'Learner Agreement' THEN status END ORDER BY FIELD(status, 'Approved', 'Verified', 'Declined', 'Pending')), ',', 1) as learner_agreement_status
        FROM learner_document 
        WHERE documentName = 'Learner Agreement'
        GROUP BY learner_id
        LIMIT 3
    ";
    $doc_subquery_result = $conn->query($doc_subquery);
    if ($doc_subquery_result) {
        while ($doc_row = $doc_subquery_result->fetch_assoc()) {
            error_log("DEBUG Subquery: learner_id=" . $doc_row['learner_id'] . 
                     ", learner_agreement_status=" . ($doc_row['learner_agreement_status'] ?? 'NULL'));
        }
    }
    
    // Debug: Test basic learner query
    $basic_query = "SELECT ld.learnerID, ld.Name, ld.Surname FROM learnerdetails ld LIMIT 3";
    $basic_result = $conn->query($basic_query);
    if ($basic_result) {
        error_log("DEBUG: Basic learner query returned " . $basic_result->num_rows . " rows");
        while ($basic_row = $basic_result->fetch_assoc()) {
            error_log("DEBUG Basic: learnerID=" . $basic_row['learnerID'] . ", Name=" . $basic_row['Name'] . ", Surname=" . $basic_row['Surname']);
        }
    }
    ...
    // Bind parameters
    $param_types = "iissssss"; // year(i), month(i), startDate(s), endDate(s), startDate(s), endDate(s), startDate(s), endDate(s)
    ...
    // Debug mode - show query results instead of downloading CSV
    if ($debug_csv) {
        echo "<h2>CSV Query Debug Results</h2>";
        echo "<p><strong>Parameters used:</strong></p>";
        ...
        echo "<p><strong>Total learners found: $learner_count</strong></p>";
        
        if ($learner_count > 0) {
            echo "<h4>Sample of first 10 learners:</h4>";
            echo "<table border='1' cellpadding='5' cellspacing='0'>";
            echo "<tr><th>LearnerID</th><th>Name</th><th>Surname</th><th>BankName</th><th>Days Attended</th><th>Document Status</th></tr>";
            ...
            while ($row = $result->fetch_assoc() && $count < 10) {
                ...
                echo "<td>" . htmlspecialchars($row['days_attended_in_month']) . "</td>";
                echo "<td>" . htmlspecialchars($row['learner_agreement_status'] ?? 'Missing') . "</td>";
                echo "</tr>";
                $count++;
            }
            echo "</table>";
            ...
            $attendance_stats = ['zero' => 0, 'low' => 0, 'medium' => 0, 'high' => 0];
            while ($row = $result->fetch_assoc()) {
                $days = intval($row['days_attended_in_month']);
                if ($days == 0) $attendance_stats['zero']++;
                elseif ($days <= 5) $attendance_stats['low']++;
                elseif ($days <= 15) $attendance_stats['medium']++;
                else $attendance_stats['high']++;
            }
            
            echo "<h4>Attendance Distribution:</h4>";
            echo "<p>0 days: {$attendance_stats['zero']} learners</p>";
            echo "<p>1-5 days: {$attendance_stats['low']} learners</p>";
            echo "<p>6-15 days: {$attendance_stats['medium']} learners</p>";
            echo "<p>16+ days: {$attendance_stats['high']} learners</p>";
        }
    ...
    $filename = "Payment_CSV_All_Projects_{$month_name}_{$year}.csv";
    ...
    // CSV Headers for payment format
    $headers = [
        'Number',
        'Cr Account Name',
        'Cr Account Number', 
        'Cr Branch Code',
        'CR Statement Reference',
        'DR Account Name',
        'DR Account Number',
        'DR Branch Number',
        'DR Statement Reference',
        'Date',
        'Amount'
    ];
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if (empty($row['fullname']) || empty($row['BankAccount']) || empty($row['bankCode']) || 
            empty($row['IDNumber']) || empty($row['amount_due'])) {
            continue;
        }

        // Format ID Number to preserve leading zeros
        $idNumber = (string)$row['IDNumber'];
           // If it's in scientific notation, fix it
        if (preg_match('/^[0-9]+\\.?[0-9]*[Ee][+\\-]?[0-9]+$/', $idNumber)) {
            $idNumber = sprintf('%.0f', (float)$idNumber);
        }

    // Use the last day of the specified month and year for the date
    $reportDate = date('Ymd', mktime(0, 0, 0, $month + 1, 0, $year));

    $csv_row = [
        $row_number,                                    // Number
        $row['fullname'],                              // Cr Account Name
        $row['BankAccount'],                           // Cr Account Number
        $row['bankCode'],                              // Cr Branch Code
        $row['sdp_name'] . " Stipend",                 // CR Statement Reference
        $dr_account_name,                              // DR Account Name
        $dr_account_number,                            // DR Account Number
        $dr_branch_number,                             // DR Branch Number
        $idNumber,                                     // DR Statement Reference
        $reportDate,                                   // Date
        $amount = number_format((float)$row['amount_due'], 2, ',', '')  // Amount with R prefix
    ];
    ...
    error_log("CSV Payment Download: Project ID: $project_id, Year: $year, Month: $month");

    exit;
}


// Function to generate agreement content
function generateAgreementContent($learner) {
    $content = "LEARNER AGREEMENT\n";
    $content .= "================\n\n";
    
    // Personal details section
    $content .= "LEARNER NAME AND SURNAME: " . ($learner['Name'] ?? 'N/A') . " " . ($learner['Surname'] ?? 'N/A') . "\n";
    $content .= "ID NUMBER: " . ($learner['IDNumber'] ?? 'N/A') . "\n";
    $content .= "CONTACT NUMBER: " . ($learner['PhoneNumber'] ?? 'N/A') . "\n";
    $content .= "LEARNER ID: " . ($learner['LearnerID'] ?? 'N/A') . "\n\n";
    
    // Qualification details
    $content .= "QUALIFICATION TITLE: NATIONAL CERTIFICATE: " . ($learner['qualification_name'] ?? 'N/A') . "\n";
    $content .= "QUALIFICATION ID: " . ($learner['qualification_id'] ?? 'N/A') . "\n";
    $content .= "LEARNING PATHWAY: " . ($learner['pathway_name'] ?? 'N/A') . "\n\n";
    
    // Programme dates
    $content .= "PROGRAMME DATES:\n";
    $content .= "Start Date: " . ($learner['pathway_start_date'] ?? 'TBD') . "\n";
    $content .= "End Date: " . ($learner['pathway_end_date'] ?? 'TBD') . "\n\n";
    
    // Training Provider details
    $content .= "TRAINING PROVIDER DETAILS:\n";
    $content .= "Legal name: " . ($learner['sdp_name'] ?? 'N/A') . "\n";
    $content .= "Business address: 6th Floor, MTL House Pinetown 3610\n";
    $content .= "Postal address: P O Box 2061 Pinetown 3600\n";
    $content .= "Contact person: Nompumelelo Mzimela\n";
    $content .= "Telephone: 0871887960\n";
    $content .= "Fax: 086 656 2578\n\n";
    
    // Declaration
    $content .= "DECLARATION OF THE PARTIES:\n";
    $content .= "We understand that this Agreement is legally binding.\n\n";
    
    // Rights and duties section
    $content .= "RIGHTS AND DUTIES OF LEARNERS AND TRAINING PROVIDERS:\n\n";
    $content .= "RIGHTS OF THE LEARNER:\n";
    $content .= "• To receive quality education and training\n";
    $content .= "• To be treated with dignity and respect\n";
    $content .= "• To have access to learning materials and resources\n";
    $content .= "• To receive fair assessment and feedback\n";
    $content .= "• To lodge complaints through proper channels\n\n";
    
    $content .= "DUTIES OF THE LEARNER:\n";
    $content .= "• To attend all scheduled training sessions\n";
    $content .= "• To complete all assignments and assessments\n";
    $content .= "• To maintain professional conduct\n";
    $content .= "• To respect fellow learners and staff\n";
    $content .= "• To follow all institutional policies and procedures\n\n";
    
    // Signature section
    $content .= "SIGNATURES:\n";
    $content .= "================\n\n";
    
    $content .= "Learner Signature: " . (!empty($learner['signature']) ? "✓ COMPLETED" : "✗ PENDING") . "\n";
    $content .= "Learner Initials: " . ($learner['learner_initials'] ?: "Not provided") . "\n";
    $content .= "Date: " . date('d F Y') . "\n\n";
    
    $content .= "Training Provider Signature: " . (!empty($learner['signature_image']) ? "✓ COMPLETED" : "✗ PENDING") . "\n";
    $content .= "Date: " . date('d F Y') . "\n\n";
    
    $content .= "WITNESSES:\n";
    $content .= "Witness Signature: " . (!empty($learner['witness_signature']) ? "✓ COMPLETED" : "✗ PENDING") . "\n";
    $content .= "Witness Initials: " . ($learner['witness_initials'] ?: "Not provided") . "\n";
    $content .= "Date: " . date('d F Y') . "\n\n";
    
    // Agreement number
    $content .= "Agreement No: " . ($learner['LearnerID'] ?? 'N/A') . "-" . date('Y') . "\n\n";
    
    // Status summary
    $content .= "AGREEMENT STATUS SUMMARY:\n";
    $content .= "=======================\n";
    $content .= "Overall Status: " . (allSignaturesComplete($learner) ? "FULLY COMPLETED ✓" : "INCOMPLETE ✗") . "\n";
    $content .= "All Required Signatures: " . (allSignaturesComplete($learner) ? "YES" : "NO") . "\n";
    $content .= "Generated: " . date('Y-m-d H:i:s') . "\n";
    
    return $content;
}

// Function to check if all signatures are complete
function allSignaturesComplete($learner) {
    return !empty($learner['signature']) && 
           !empty($learner['learner_initials']) && 
           !empty($learner['witness_signature']) && 
           !empty($learner['witness_initials']);
}

// Handle Selected Excel Export
if (isset($_POST['export_selected_excel']) && $_POST['export_selected_excel'] == '1') {
        // Get parameters
        $project_id = filter_input(INPUT_POST, 'project_id', FILTER_VALIDATE_INT);
        if (!$project_id) { $project_id = $session_project_id ?: 76; }
        $year = filter_input(INPUT_POST, 'year', FILTER_VALIDATE_INT);
        $month = filter_input(INPUT_POST, 'month', FILTER_VALIDATE_INT);
        $selectedLearnersJson = $_POST['selected_learners'] ?? '';
        
        // Get DR account details (optional)
        $dr_account_name = filter_input(INPUT_POST, 'dr_account_name', FILTER_SANITIZE_FULL_SPECIAL_CHARS) ?: '';
        $dr_account_number = filter_input(INPUT_POST, 'dr_account_number', FILTER_SANITIZE_FULL_SPECIAL_CHARS) ?: '';
        $dr_branch_number = filter_input(INPUT_POST, 'dr_branch_number', FILTER_SANITIZE_FULL_SPECIAL_CHARS) ?: '';
        
        if (!$project_id || !$year || !$month || empty($selectedLearnersJson)) {
            die("Error: Missing required parameters for selected Excel export.");
        }
    
        // Decode selected learners
        $selectedLearners = json_decode($selectedLearnersJson, true);
        if (!$selectedLearners) {
        die("Error: Invalid selected learners data.");
        }
    
        // Get project and SDP names for file naming
        $project_query = "SELECT Project_name FROM project WHERE project_id = ?";
        $stmt = $conn->prepare($project_query);
        $stmt->bind_param('i', $project_id);
        $stmt->execute();
        ...
        // Get SDP name using project ID
        $sdp_query = "SELECT s.sdp_name FROM sdp s 
                      JOIN sites si ON s.sdp_id = si.sdp_id 
                      WHERE si.project_id = ? 
                      LIMIT 1";
        $stmt = $conn->prepare($sdp_query);
        $stmt->bind_param('i', $project_id);
        $stmt->execute();
        $sdp_result = $stmt->get_result();
        $sdp = $sdp_result->fetch_assoc();
        $sdp_name = $sdp['sdp_name'] ?? 'Unknown_SDP';
    
    // Create filename
    $month_name = date('F', mktime(0, 0, 0, $month, 1));
    $filename = "Selected_Learners_{$project_name}_{$sdp_name}_{$month_name}_{$year}.xls";
        ...
        // Excel Headers for payment format
        $headers = [
            'Number',
            'Cr Account Name',
            'Cr Account Number', 
    ...
    // Get learner details from database for selected learners
    $idNumbers = array_column($selectedLearners, 'id_number');
    $placeholders = str_repeat('?,', count($idNumbers) - 1) . '?';
    
    $learnerDetailsQuery = "SELECT DISTINCT
        ld.IDNumber,
        ld.Surname,
        ld.Name,
        CONCAT(ld.Surname, ' ', ld.Name) AS fullname,
        ...
        bd.BankAccount,
        bd.bankCode,
        ld_doc.status as learner_agreement_status,
        sn.status as sick_note_status
    FROM learnerdetails ld
    JOIN bankdetails bd ON bd.LearnerID = ld.learnerID
    LEFT JOIN (
        SELECT 
            learner_id,
            documentName,
        ...
        FROM learner_document 
        WHERE documentName = 'Learner Agreement'
        GROUP BY learner_id, documentName
    ) ld_doc ON ld.LearnerID = ld_doc.learner_id
    LEFT JOIN (
        SELECT DISTINCT learner_id, status
        FROM sick_note 
        WHERE MONTH(date_from) = ? 
        AND YEAR(date_from) = ?
    ) sn ON ld.LearnerID = sn.learner_id
    WHERE ld.IDNumber IN ($placeholders)";
    
    $stmt = $conn->prepare($learnerDetailsQuery);
    $params = array_merge([$month, $year], $idNumbers);
    $paramTypes = str_repeat('s', count($params));
    $stmt->bind_param($paramTypes, ...$params);
    $stmt->execute();
    $learnerDetails = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
    
    // Create lookup array for learner details
    $learnerLookup = [];
    foreach ($learnerDetails as $detail) {
        $learnerLookup[$detail['IDNumber']] = $detail;
    }
    
    // Write selected learners data in Excel format
    $rowNumber = 2;
    foreach ($selectedLearners as $index => $learner) {
        $detail = $learnerLookup[$learner['id_number']] ?? null;
        
        if (!$detail || empty($detail['fullname']) || empty($detail['BankAccount']) || empty($detail['bankCode'])) {
            continue; // Skip learners with missing critical data
        }
        ...
        $excel_row = [
            $index + 1,                                    // Number
            $detail['fullname'],                           // Cr Account Name
            $detail['BankAccount'],                        // Cr Account Number
            $detail['bankCode'],                           // Cr Branch Code
            $sdp_name . " Stipend",                        // CR Statement Reference
            $dr_account_name,                              // DR Account Name
            $dr_account_number,                            // DR Account Number
            $dr_branch_number,                             // DR Branch Number
            '="' . $idNumber . '"',                        // DR Statement Reference (Excel format to preserve leading zeros)
            $reportDate,                                   // Date  
            number_format((float)$learner['amount'], 2, ',', '')  // Amount with comma decimal separator
        ];
        
        // Write row to Excel
        $sheet->fromArray($excel_row, NULL, 'A' . $rowNumber);
        $rowNumber++;
    }
...
// Handle AJAX request to check for pending sick notes
if (isset($_GET['check_sick_notes']) && $_GET['check_sick_notes'] == '1') {
    ...
    if ($project_id === false || $year === false || $month === false) {
        echo json_encode(['has_pending_sick_notes' => false]);
        exit;
    }
    ...
    echo json_encode(['has_pending_sick_notes' => $has_pending_sick_notes]);
    exit;
}
...
$districts = [];
if ($selectedProject) {
    $stmt = $conn->prepare("SELECT DISTINCT District FROM sites WHERE project_id = ? ORDER BY District");
    if ($stmt) {
...
if ($selectedProject) {
    if (!empty($_GET['district'])) {
        $stmt = $conn->prepare("SELECT DISTINCT siteName FROM sites WHERE project_id = ? AND District = ? ORDER BY siteName");
        if ($stmt) {
            $districtParam = $_GET['district'];
            $stmt->bind_param('is', $selectedProject, $districtParam);
            $stmt->execute();
        ...
        $stmt = $conn->prepare("SELECT DISTINCT siteName FROM sites WHERE project_id = ? ORDER BY siteName");
        if ($stmt) {
            $stmt->bind_param('i', $selectedProject);
            $stmt->execute();
            $siteResult = $stmt->get_result();
...
$selectedProject = isset($_GET['selected_project']) ? (int)$_GET['selected_project'] : $session_project_id;
...
if (!empty($selectedSite)) {
    if ($selectedProject) {
        $stmt = $conn->prepare("SELECT c.classID, c.className FROM class c JOIN sites s ON c.siteID = s.siteID WHERE s.siteName = ? AND s.project_id = ? ORDER BY c.className");
        if ($stmt) {
            $stmt->bind_param('si', $selectedSite, $selectedProject);
            $stmt->execute();
        ...
        $stmt = $conn->prepare("SELECT c.classID, c.className FROM class c JOIN sites s ON c.siteID = s.siteID WHERE s.siteName = ? ORDER BY c.className");
        if ($stmt) {
            $stmt->bind_param('s', $selectedSite);
            $stmt->execute();
            $classResult = $stmt->get_result();
            while ($row = $classResult->fetch_assoc()) {
                $classes[] = $row;
...
$documentStatusFilter = isset($_GET['document_status']) ? $_GET['document_status'] : '';
$signatureStatusFilter = isset($_GET['signature_status']) ? $_GET['signature_status'] : '';
$documentTypeFilter = isset($_GET['document_type']) ? $_GET['document_type'] : '';
...
// Build the main query with activity_status filter and sick days calculation
// FIXED: Separate learner data from attendance data to prevent signature mixing
$sql = "
    SELECT
        s.District,
        s.project_id,
        ...
        ld.witness_initials,
        ld.activity_statu,
        ld_doc.learner_agreement_status,
        bd.BankName,
        bd.BankAccount,
        sn.status as sick_note_status,
        COALESCE(sick_days.approved_sick_days, 0) as approved_sick_days
    ...
    LEFT JOIN bankdetails bd ON ld.LearnerID = bd.LearnerID
    LEFT JOIN (
        SELECT DISTINCT learner_id, status
        FROM sick_note 
        WHERE DATE(date_from) BETWEEN '{$startDate}' AND '{$endDate}'
    ) sn ON ld.LearnerID = sn.learner_id
    LEFT JOIN (
        SELECT 
            sn.learner_id,
            COUNT(DISTINCT sn.date_from) as approved_sick_days
        FROM sick_note sn
        WHERE DATE(sn.date_from) BETWEEN '{$startDate}' AND '{$endDate}'
        AND sn.status IN ('Approved', 'N/A')
        AND DAYOFWEEK(sn.date_from) NOT IN (1, 7)
        GROUP BY sn.learner_id
    ) sick_days ON sick_days.learner_id = ld.LearnerID
    WHERE (ld.activity_statu = '' OR ld.activity_statu IS NULL)
...
if (!empty($selectedProject)) {
    $whereClauses[] = "s.project_id = '" . $conn->real_escape_string($selectedProject) . "'";
}
...
if (!empty($selectedClass)) {
    $whereClauses[] = "c.classID = '" . $conn->real_escape_string($selectedClass) . "'";
}
if (isset($_GET['id_search']) && !empty($_GET['id_search'])) {
    $whereClauses[] = "ld.IDNumber = '" . $conn->real_escape_string($_GET['id_search']) . "'";
}
// DISABLED: Always exclude learners with pending sick notes (to show all 1855 learners)
// $whereClauses[] = "(sn.status IS NULL OR sn.status != 'Pending')";

// Apply document type filter first (if selected)
if (!empty($documentTypeFilter)) {
    // Filter by specific document type - show all statuses for that document type
    switch($documentTypeFilter) {
        case 'Learner Agreement':
            if (!empty($documentStatusFilter)) {
                if ($documentStatusFilter === 'Missing') {
                    $whereClauses[] = "(ld_doc.learner_agreement_status IS NULL OR ld_doc.learner_agreement_status = '' OR ld_doc.learner_agreement_status = 'Missing')";
                } else {
                    $whereClauses[] = "ld_doc.learner_agreement_status = '" . $conn->real_escape_string($documentStatusFilter) . "'";
                }
            }
            break;
    }
} else {
    // No document type filter selected - show Learner Agreement with status filtering
    if (!empty($documentStatusFilter)) {
        if ($documentStatusFilter === 'Missing') {
            // Show learners where Learner Agreement is missing
            $whereClauses[] = "(ld_doc.learner_agreement_status IS NULL OR ld_doc.learner_agreement_status = '' OR ld_doc.learner_agreement_status = 'Missing')";
        } else {
            // Apply status filter to Learner Agreement
            $whereClauses[] = "ld_doc.learner_agreement_status = '" . $conn->real_escape_string($documentStatusFilter) . "'";
        }
    }
}
...
if (!empty($signatureStatusFilter)) {
    if ($signatureStatusFilter === 'complete') {
        // Show learners with all signature fields completed
        $whereClauses[] = "(ld.signature IS NOT NULL AND ld.signature != '' AND 
                           ld.learner_initials IS NOT NULL AND ld.learner_initials != '' AND 
                           ld.witness_signature IS NOT NULL AND ld.witness_signature != '' AND 
                           ld.witness_initials IS NOT NULL AND ld.witness_initials != '')";
    } elseif ($signatureStatusFilter === 'incomplete') {
        // Show learners with missing signature fields
        $whereClauses[] = "(ld.signature IS NULL OR ld.signature = '' OR 
                           ld.learner_initials IS NULL OR ld.learner_initials = '' OR 
                           ld.witness_signature IS NULL OR ld.witness_signature = '' OR 
                           ld.witness_initials IS NULL OR ld.witness_initials = '')";
    }
}
...
if (isset($_GET['debug']) || isset($_GET['debug_missing'])) {
    echo "<div style='background: #e7f3ff; border: 1px solid #b3d9ff; padding: 10px; margin: 10px 0; border-radius: 5px;'>";
    echo "<h5>🔍 Applied Filters Debug:</h5>";
    echo "<p><strong>Selected Project:</strong> " . ($selectedProject ?: 'None') . "</p>";
    echo "<p><strong>Selected District:</strong> " . ($selectedDistrict ?: 'None') . "</p>";
    echo "<p><strong>Selected Site:</strong> " . ($selectedSite ?: 'None') . "</p>";
    echo "<p><strong>Document Status Filter:</strong> " . ($documentStatusFilter ?: 'None') . "</p>";
    echo "<p><strong>Signature Status Filter:</strong> " . ($signatureStatusFilter ?: 'None') . "</p>";
    echo "<p><strong>Document Type Filter:</strong> " . ($documentTypeFilter ?: 'None') . "</p>";
    echo "<p><strong>ID Search:</strong> " . ($_GET['id_search'] ?? 'None') . "</p>";
    echo "<p><strong>WHERE Clauses Applied:</strong> " . (empty($whereClauses) ? 'None' : implode(' AND ', $whereClauses)) . "</p>";
    echo "</div>";
}
...
error_log("Query executed successfully. Rows found: " . $result->num_rows);
...
// Define rates - calculate daily rate based on full month
$fullMonthWorkingDays = 21; // Standard working days in a month (excluding weekends)
$perfectAttendanceAmount = 2000.00; // Amount for 100% attendance in full month
$dailyRate = $perfectAttendanceAmount / $fullMonthWorkingDays; // Daily rate = 2000/21 ≈ 95.24

if ($result && $result->num_rows > 0) {
    error_log("Processing " . $result->num_rows . " rows from database");
    $debugCount = 0;
    
    // Debug: Check if we have any results at all
    error_log("MAIN QUERY returned " . $result->num_rows . " rows");
    
    // Store debug info for display on page
    $mainQueryDebugInfo = [
        'total_rows' => $result->num_rows,
        'sample_rows' => []
    ];
    
    // FIXED: Process learner data first (no attendance mixing)
    while ($row = $result->fetch_assoc()) {
        // Debug: Log the first few rows to see what data we're getting
        if ($debugCount < 3) {
            error_log("DEBUG Row $debugCount: LearnerID=" . ($row['LearnerID'] ?? 'NULL') . 
                     " | Signature=" . ($row['signature'] ?? 'NULL') . 
                     " | Learner Agreement Status=" . ($row['learner_agreement_status'] ?? 'NULL'));
            
            // Store sample data for page display
            $mainQueryDebugInfo['sample_rows'][] = [
                'learner_id' => $row['LearnerID'] ?? 'NULL',
                'name' => $row['Name'] ?? 'NULL',
                'surname' => $row['Surname'] ?? 'NULL',
                'signature' => $row['signature'] ?? 'NULL',
                'learner_agreement_status' => $row['learner_agreement_status'] ?? 'NULL'
            ];
            $debugCount++;
        }
        $learnerKey = $row['LearnerID'];
        
        // Create learner record with correct signature data
        $reportData[$learnerKey] = [
            'name' => $row['Name'],
            'surname' => $row['Surname'],
            'id_number' => $row['IDNumber'],
            'document_name' => $row['documentName'] ?? 'N/A',
            'document_status' => $row['document_status'] ?? 'N/A',
            'bank_name' => $row['BankName'] ?? 'N/A',
            'bank_account' => $row['BankAccount'] ?? 'N/A',
            'sick_note_status' => $row['sick_note_status'] ?? 'N/A',
            'approved_sick_days' => $row['approved_sick_days'] ?? 0,
            ...
            // Document statuses
            'learner_agreement_status' => $row['learner_agreement_status'] ?? 'Missing',
            'attendance' => []
        ];
        error_log("Added learner: " . $row['Name'] . " " . $row['Surname'] . " (ID: " . $learnerKey . ") with signature: " . ($row['signature'] ?? 'NULL'));
    }
    
    // FIXED: Now get attendance data separately to avoid signature mixing
    $attendance_sql = "
        SELECT DISTINCT ld.LearnerID, DATE(lc.clock_date) as clock_date
        FROM sites s
        JOIN class c ON s.siteID = c.siteID
        JOIN learnerdetails ld ON c.classID = ld.classID
        JOIN learner_clocking lc ON ld.LearnerID = lc.LearnerID
        WHERE DATE(lc.clock_date) BETWEEN '{$startDate}' AND '{$endDate}'
        AND (ld.activity_statu = '' OR ld.activity_statu IS NULL)
    ";
    
    // Apply same filters to attendance query
    if (!empty($whereClauses)) {
        $attendance_sql .= " AND " . implode(" AND ", $whereClauses);
    }
    
    $attendance_result = $conn->query($attendance_sql);
    if ($attendance_result && $attendance_result->num_rows > 0) {
        while ($att_row = $attendance_result->fetch_assoc()) {
            $learnerKey = $att_row['LearnerID'];
            if (isset($reportData[$learnerKey]) && $att_row['clock_date']) {
                $reportData[$learnerKey]['attendance'][$att_row['clock_date']] = true;
            }
        }
    }
    
    error_log("Total learners in reportData: " . count($reportData));
    
    // DEBUG: Compare with baseline query to find missing learners
    if (isset($_GET['debug']) || isset($_GET['debug_missing'])) {
        echo "<div style='background: #fff3cd; border: 1px solid #ffeaa7; padding: 15px; margin: 10px 0; border-radius: 5px;'>";
        echo "<h4>🔍 DEBUGGING: Main Query vs Baseline Comparison</h4>";
        
        // Run baseline query (same as debug mode)
        $baseline_sql = "
        SELECT DISTINCT ld.LearnerID, ld.Name, ld.Surname, ld.IDNumber, si.District, si.project_id
        FROM sites si
        JOIN class c ON si.siteID = c.siteID  
        JOIN learnerdetails ld ON c.classID = ld.classID
        WHERE (ld.activity_statu = '' OR ld.activity_statu IS NULL)
        ORDER BY ld.Surname, ld.Name
        ";
        
        $baseline_stmt = $conn->prepare($baseline_sql);
        $baseline_stmt->execute();
        $baseline_result = $baseline_stmt->get_result();
        $baseline_learners = [];
        while ($row = $baseline_result->fetch_assoc()) {
            $baseline_learners[$row['LearnerID']] = $row;
        }
        
        // Get learners from main query
        $main_learners = array_keys($reportData);
        
        echo "<p><strong>Baseline Query Results:</strong> " . count($baseline_learners) . " learners</p>";
        echo "<p><strong>Main Query Results:</strong> " . count($reportData) . " learners</p>";
        echo "<p><strong>Missing from Main Query:</strong> " . (count($baseline_learners) - count($reportData)) . " learners</p>";
        
        // Find missing learners
        $missing_learners = [];
        foreach ($baseline_learners as $learner_id => $learner_data) {
            if (!in_array($learner_id, $main_learners)) {
                $missing_learners[$learner_id] = $learner_data;
            }
        }
        
        if (!empty($missing_learners)) {
            echo "<h5>❌ Missing Learners (" . count($missing_learners) . "):</h5>";
            echo "<table style='width: 100%; border-collapse: collapse; font-size: 12px;'>";
            echo "<tr style='background: #f8f9fa;'><th style='border: 1px solid #ddd; padding: 5px;'>ID</th><th style='border: 1px solid #ddd; padding: 5px;'>Name</th><th style='border: 1px solid #ddd; padding: 5px;'>ID Number</th><th style='border: 1px solid #ddd; padding: 5px;'>District</th><th style='border: 1px solid #ddd; padding: 5px;'>Project ID</th></tr>";
            foreach (array_slice($missing_learners, 0, 20) as $learner_id => $learner) {
                echo "<tr>";
                echo "<td style='border: 1px solid #ddd; padding: 5px;'>{$learner_id}</td>";
                echo "<td style='border: 1px solid #ddd; padding: 5px;'>{$learner['Name']} {$learner['Surname']}</td>";
                echo "<td style='border: 1px solid #ddd; padding: 5px;'>{$learner['IDNumber']}</td>";
                echo "<td style='border: 1px solid #ddd; padding: 5px;'>{$learner['District']}</td>";
                echo "<td style='border: 1px solid #ddd; padding: 5px;'>{$learner['project_id']}</td>";
                echo "</tr>";
            }
            if (count($missing_learners) > 20) {
                echo "<tr><td colspan='5' style='border: 1px solid #ddd; padding: 5px; text-align: center; font-style: italic;'>... and " . (count($missing_learners) - 20) . " more</td></tr>";
            }
            echo "</table>";
        } else {
            echo "<p style='color: green;'>✅ No missing learners found!</p>";
        }
        
        echo "</div>";
    }
} else {
    error_log("No rows found in query result");
}
...
foreach ($reportData as $learnerID => $learner) {
    ...
    // Calculate days affected (attendance + approved sick days)
    $approvedSickDays = $learner['approved_sick_days'] ?? 0;
    $daysAffected = $totalPresent + $approvedSickDays;
    ...
    if ($includeInFilter) {
        ...
        // Calculate amount due based on selected date range, not full month
        $selectedWorkingDays = count($dates); // This is already the selected date range working days
        $daysAffectedPercent = $selectedWorkingDays > 0 ? round(($daysAffected / $selectedWorkingDays) * 100, 1) : 0;
        
        // Calculate amount based on selected date range
        if ($daysAffectedPercent == 100) {
            // If 100% attendance in selected range, calculate full amount for that range
            $amountDue = $selectedWorkingDays * $dailyRate;
        } else {
            // Use daily rate for actual days attended in selected range
            $amountDue = $daysAffected * $dailyRate;
        }
...
error_log("After filtering - Total learners: " . $totalLearners . ", ReportData count: " . count($reportData));
...
if (isset($_GET['export_excel']) && $_GET['export_excel'] == '1') {
    ...
    // Set headers
    $headers = [
        '#', 'Surname', 'Name', 'ID Number', 'Phone Number', 'Document Name', 'Learner Agreement Status', 'Signature', 'Learner Initials', 'Witness Signature', 'Witness Initials', 'Bank Name', 'Bank Account', 'Sick Note Status'
    ];
    foreach ($dates as $date) {
        $headers[] = date('M d D', strtotime($date));
    }
    $headers[] = 'Total Present';
    $headers[] = 'Days Affected';
    $headers[] = 'Attendance %';
    $headers[] = 'Daily Rate';
    $headers[] = 'Amount Due';
    ...
    foreach ($reportData as $learnerID => $learner) {
        ...
        // Calculate days affected (attendance + approved/N/A sick days only)
        // Note: Rejected sick days are treated as absent, Pending learners are excluded entirely (treated as N/A)
        $approvedSickDays = $learner['approved_sick_days'] ?? 0;
        $daysAffected = $totalPresent + $approvedSickDays;
        ...
        $rowData = [
            ...
            $learner['document_name'] ?? 'N/A',
            $learner['learner_agreement_status'] ?? 'N/A',
            !empty($learner['signature']) ? 'Signed' : 'Not Signed',
            $learner['learner_initials'] ?? 'N/A',
            !empty($learner['witness_signature']) ? 'Signed' : 'Not Signed',
            $learner['witness_initials'] ?? 'N/A',
            $learner['bank_name'] ?? 'N/A',
            $learner['bank_account'] ?? 'N/A',
            $learner['sick_note_status'] ?? 'N/A'
        ];
        foreach ($dates as $date) {
            $rowData[] = isset($learner['attendance'][$date]) ? 'Present' : 'Absent';
        }
        $rowData[] = $totalPresent . '/' . count($dates);
        $rowData[] = $daysAffected;
...
if (isset($_GET['export_pdf']) && $_GET['export_pdf'] == '1') {
    ...
    foreach ($reportData as $learnerID => $learner) {
        ...
        $html .= '<td>' . htmlspecialchars($learner['document_name'] ?? 'N/A', ENT_QUOTES, 'UTF-8') . '</td>';
        $html .= '<td>' . htmlspecialchars($learner['learner_agreement_status'] ?? 'N/A', ENT_QUOTES, 'UTF-8') . '</td>';
        $html .= '<td>' . (!empty($learner['signature']) ? 'Signed' : 'Not Signed') . '</td>';
        $html .= '<td>' . htmlspecialchars($learner['learner_initials'] ?? 'N/A', ENT_QUOTES, 'UTF-8') . '</td>';
        $html .= '<td>' . (!empty($learner['witness_signature']) ? 'Signed' : 'Not Signed') . '</td>';
        $html .= '<td>' . htmlspecialchars($learner['witness_initials'] ?? 'N/A', ENT_QUOTES, 'UTF-8') . '</td>';
        $html .= '<td>' . htmlspecialchars($learner['bank_name'] ?? 'N/A', ENT_QUOTES, 'UTF-8') . '</td>';
        $html .= '<td>' . htmlspecialchars($learner['bank_account'] ?? 'N/A', ENT_QUOTES, 'UTF-8') . '</td>';
        $html .= '<td>' . htmlspecialchars($learner['sick_note_status'] ?? 'N/A', ENT_QUOTES, 'UTF-8') . '</td>';
        ...
        $html .= '<td class="stats-cell">' . $daysAffected . '</td>';
...
// Function to detect valid witness signatures for a learner
function detectValidWitnessSignature($conn, $learnerID) {
    $validSignature = null;
    $hasDatabaseSignature = false;

    error_log("=== WITNESS SIGNATURE DETECTION DEBUG START ===");
    error_log("Looking for witness signatures for learner ID: $learnerID");

    // First, let's check what witness signatures exist in the database for this learner
    $debugQuery = "
        SELECT
    ...
    if ($debugStmt = $conn->prepare($debugQuery)) {
        $debugStmt->bind_param("i", $learnerID);
        $debugStmt->execute();
        $debugResult = $debugStmt->get_result();

        error_log("Database witness signature check results:");
        while ($debugRow = $debugResult->fetch_assoc()) {
            error_log("- Table: " . $debugRow['table_name'] . ", Witness Signature: " . ($debugRow['witness_signature'] ?? 'NULL') . ", LearnerID: " . $debugRow['LearnerID']);
            if (!empty($debugRow['witness_signature'])) {
                $hasDatabaseSignature = true;
            }
        }
        $debugStmt->close();
    }
    ...
    // Try to find the witness signature file
    if ($hasDatabaseSignature) {
        $witnessSignatureQuery = "SELECT witness_signature FROM learnerdetails WHERE LearnerID = ?";
        if ($stmt = $conn->prepare($witnessSignatureQuery)) {
            $stmt->bind_param("i", $learnerID);
            $stmt->execute();
            $result = $stmt->get_result();
            
            if ($row = $result->fetch_assoc()) {
                $witnessSignatureFile = $row['witness_signature'];
                if (!empty($witnessSignatureFile)) {
                    error_log("Found witness signature filename in database: $witnessSignatureFile");
                    
                    if (preg_match('#^https?://#', $witnessSignatureFile)) {
                        $validSignature = $witnessSignatureFile;
                    } else {
                        $basename = basename($witnessSignatureFile);
                        foreach ($possiblePathsBase as $prefix) {
                            $candidates = [
                                $prefix . $witnessSignatureFile,
                                $prefix . $basename
                            ];
                            foreach ($candidates as $fullPath) {
                                if (file_exists($fullPath)) {
                                    error_log("Found witness signature file at: $fullPath");
                                    $validSignature = $fullPath;
                                    break 2;
                                } else {
                                    error_log("Witness signature file not found at: $fullPath");
                                }
                            }
                        }
                    }
    ...
    error_log("Final witness signature result: " . ($validSignature ?? 'NO WITNESS SIGNATURE FOUND'));
    error_log("Has database witness signature: " . ($hasDatabaseSignature ? 'YES' : 'NO'));
    error_log("=== WITNESS SIGNATURE DETECTION DEBUG END ===");

    return $validSignature;
}

// Function to detect valid signatures for a learner - FIXED VERSION
function detectValidSignature($conn, $learnerID) {
    ...
    // FIXED: More precise query that ensures we only get signatures for THIS specific learner
    $learnerSignatureQuery = "
        SELECT signature, learner_initials, witness_signature, witness_initials
        FROM learnerdetails 
        WHERE LearnerID = ? 
        AND (signature IS NOT NULL AND signature != '' AND signature != 'N/A')
        LIMIT 1
    ";

    $stmt = $conn->prepare($learnerSignatureQuery);
    if ($stmt) {
        $stmt->bind_param('i', $learnerID);
        $stmt->execute();
        $result = $stmt->get_result();
        ...
        if ($result->num_rows > 0) {
            ...
            error_log("Found signature in learnerdetails for learner $learnerID: $signature");
            ...
            error_log("Checking possible paths for signature:");
            foreach ($possiblePaths as $path) {
                error_log("- Checking: $path");
                if (file_exists($path)) {
                    error_log("✓ FOUND SIGNATURE FILE: $path");
                    $validSignature = $path;
                    break;
                }
            }
    ...
    // FIXED: Only check learner_clocking if no signature found in learnerdetails
    // AND ensure we're only getting signatures for THIS specific learner
    if (!$validSignature) {
        error_log("No signature found in learnerdetails, checking learner_clocking for learner $learnerID");
        
        $clockingSignatureQuery = "
        ...
        $stmt2 = $conn->prepare($clockingSignatureQuery);
        if ($stmt2) {
            ...
            if ($result2->num_rows > 0) {
                ...
                error_log("Found signature in learner_clocking for learner $learnerID: $signature");
    ...
    // FIXED: Removed the broad file search that could pick up wrong signatures
    // Only use database-referenced signatures to ensure accuracy
    
    error_log("Final signature result for learner $learnerID: " . ($validSignature ?? 'NO SIGNATURE FOUND'));
            ...
            if (isset($clockingData[$date])) {
                ...
                if ($isCompleteRecord) {
                    ...
                    if ($webSignaturePath) {
                        ...
                        $html .= "<img src='$webSignaturePath' class='signature-img rounded' alt='Signature' style='width: 100px; height: 50px; border: 1px solid #ddd; border-radius: 4px;' onerror=\"this.style.display='none';this.nextSibling.style.display='block; console.log('Signature failed to load: $webSignaturePath');\">";
            ...
            if ($isApprovedSick) {
                ...
                if (isset($clockingData[$date])) {
                    ...
                    if ($isCompleteRecord) {
                        ...
                        if ($webSignaturePath) {
                            $webSignaturePath = htmlspecialchars($webSignaturePath);
                            $html .= "<img src='$webSignaturePath' class='signature-img rounded' alt='Signature' style='width: 100px; height: 50px;' onerror=\"this.style.display='none';this.nextSibling.style.display='block';\">";
                            $html .= "<small style='display:none;'>Signature: [N/A]</small>";
                        } else {
...
if (isset($_GET['export_pdf_bulk']) && $_GET['export_pdf_bulk'] == '1') {
    ...
    echo "<form action='" . htmlspecialchars($_SERVER['PHP_SELF']) . "' method='GET' style='margin:0; display:flex; gap:8px; align-items:center;'>";
    echo "<input type='hidden' name='forms_zip' value='1'>";
    echo "<input type='number' name='learner_id' placeholder='LearnerID' style='padding:8px;border:1px solid #ced4da;border-radius:4px;width:140px;' required>";
    echo "<button type='submit' style='background:#6f42c1;color:#fff;border:none;border-radius:5px;padding:10px 16px;cursor:pointer;'>📦 Generate Learner Forms ZIP</button>";
    echo "</form>";
    echo "</div>";
    // Auto-start ZIP download (with fallback click handler)
    echo "<script>
            (function(){
                try {
                    // Attempt automatic navigation to start download
                    setTimeout(function(){ window.location.href='" . addslashes($downloadUrlWithTs) . "'; }, 600);
                    // Also bind fallback on the button
                    var l=document.getElementById('zipLink');
                    if(l){ l.addEventListener('click', function(e){ try { setTimeout(function(){ window.location.href='" . addslashes($downloadUrlWithTs) . "'; }, 500); } catch(err){} }); }
                } catch(err) { /* silent */ }
            })();
          </script>";
    echo "</p>";
...
// Handle learner forms ZIP generation
if (isset($_GET['forms_zip']) && $_GET['forms_zip'] == '1' && !empty($_GET['learner_id'])) {
    $learnerId = intval($_GET['learner_id']);
    $tempFile = sys_get_temp_dir() . '/forms_' . uniqid() . '.bin';
    $fp = fopen($tempFile, 'wb');
    $isHttps = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off');
    $scheme = $isHttps ? 'https' : 'http';
    $host = $_SERVER['HTTP_HOST'] ?? '';
    $base = rtrim(dirname($_SERVER['PHP_SELF']), '/\\');
    $endpoint = $base . 'mobile/new_aggrement22.php?LearnerID=' . urlencode($learnerId);
    $fullUrl = $host ? ($scheme . '://' . $host . $endpoint) : $endpoint;
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $fullUrl);
    ...
    $zipPath = $reportsDir . '/Learner_' . $learnerId . '_Forms_' . date('Ymd_His') . '.zip';
    $isZip = ($sig === "PK\x03\x04" || $sig === "PK\x05\x06" || (is_string($ct) && stripos($ct, 'zip') !== false));
    if ($isZip) {
        ...
        if ($ok) {
            $docxData = file_get_contents($tempFile);
            $zip->addFromString('Agreement_' . $learnerId . '.docx', $docxData);
            $zip->close();
        }
        @unlink($tempFile);
    }
    ...
    die('Failed to generate learner forms ZIP.');
}
...
<html>
<head>
    <title>Learner Agreement Report</title>
    <style>
    * {
    margin: 0;
    padding: 0;
...
<body>
    <div class="sidebar">
        ...
        <div class="nav-buttons">
            <a href="admin_landingPage.php" class="nav-btn dashboard"> Back to Dashboard</a>
    ...
    <div class="main-content">
        ...
        <div class="container">
            <div class="header">
                <h1>📋 Learner Agreement Report</h1>
                <p class="subtitle">Learner Agreement document status tracking</p>
            </div>

            <!-- Document Summaries Section -->
            <div class="report-section">
                <div class="section-header">
                    <h3>📋 Learner Agreement Summary</h3>
                    <p class="section-subtitle">Overview of Learner Agreement status analytics</p>
                </div>
                <div class="document-summary-grid">
                    <?php
                    // Calculate individual document status counts
                    $documentStatusCounts = [
                        'Learner Agreement' => ['verified' => 0, 'approved' => 0, 'pending' => 0, 'declined' => 0, 'missing' => 0]
                    ];
                    
                    $documentMappings = [
                        'Learner Agreement' => 'learner_agreement_status'
                    ];
                    
                    // Count statuses for each document type
                    foreach ($reportData as $learner) {
                        foreach ($documentMappings as $docName => $docField) {
                            $status = strtolower($learner[$docField] ?? 'missing');
                            if (isset($documentStatusCounts[$docName][$status])) {
                                $documentStatusCounts[$docName][$status]++;
                            }
                        }
                    }
                    ...
                    foreach ($documentStatusCounts as $docType => $statusCounts):
                        $totalCount = array_sum($statusCounts);
                    ?>
                    <div class="document-card" style="position: relative; overflow: hidden;">
                        <div class="document-icon"><?= $documentIcons[$docType] ?></div>
                        <h4><?= $docType ?></h4>
                        <p style="margin-bottom: 15px; font-size: 0.85rem; color: #666;">
                            Signed agreements and contracts with learners (compulsory)
                        </p>
                        
                        <!-- Document Status Overview for this document type -->
                        <div class="document-status-overview" style="margin-top: 15px;">
                            <h5 style="font-size: 0.9rem; margin-bottom: 10px; color: #333;">🎯 Status Overview</h5>
                            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(80px, 1fr)); gap: 8px; margin-bottom: 10px;">
                                <!-- Approved -->
                                <div style="background: linear-gradient(135deg, #16a34a, #15803d); color: white; padding: 8px; border-radius: 8px; text-align: center;">
                                    <div style="font-size: 0.7rem; opacity: 0.9;">Approved</div>
                                    <div style="font-size: 1.2rem; font-weight: bold;"><?= $statusCounts['approved'] ?></div>
                                </div>
                                <!-- Pending -->
                                <div style="background: linear-gradient(135deg, #eab308, #ca8a04); color: white; padding: 8px; border-radius: 8px; text-align: center;">
                                    <div style="font-size: 0.7rem; opacity: 0.9;">Pending</div>
                                    <div style="font-size: 1.2rem; font-weight: bold;"><?= $statusCounts['pending'] ?></div>
                                </div>
                                <!-- Declined -->
                                <div style="background: linear-gradient(135deg, #dc2626, #b91c1c); color: white; padding: 8px; border-radius: 8px; text-align: center;">
                                    <div style="font-size: 0.7rem; opacity: 0.9;">Declined</div>
                                    <div style="font-size: 1.2rem; font-weight: bold;"><?= $statusCounts['declined'] ?></div>
                                </div>
                                <!-- Missing -->
                                <div style="background: linear-gradient(135deg, #dc2626, #b91c1c); color: white; padding: 8px; border-radius: 8px; text-align: center;">
                                    <div style="font-size: 0.7rem; opacity: 0.9;">Missing</div>
                                    <div style="font-size: 1.2rem; font-weight: bold;"><?= $statusCounts['missing'] ?></div>
                                </div>
                            </div>
                            
                            <!-- Summary Stats -->
                            <div style="background: #f8fafc; padding: 8px; border-radius: 6px; font-size: 0.75rem;">
                                ...
                                <div style="display: flex; justify-content: space-between;">
                                    <span>Complete:</span>
                                    <strong style="color: #16a34a;"><?= $statusCounts['approved'] ?> (<?= $totalCount > 0 ? round(($statusCounts['approved'] / $totalCount) * 100, 1) : 0 ?>%)</strong>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="filters">
                ...
                <form method="GET" id="filterForm">
                    <input type="hidden" name="project_id" value="<?= htmlspecialchars($session_project_id ?? '', ENT_QUOTES, 'UTF-8') ?>" id="project_id">
                    
                    <!-- Project Selection Dropdown -->
                    <div class="filter-group">
                        <label class="filter-label" for="selected_project">Project</label>
                        <select name="selected_project" id="selected_project" onchange="handleProjectChange()">
                    ...
                    <div class="filter-group">
                        ...
                        <select name="class" id="class">
                            ...
                            <?php foreach ($classes as $class): ?>
                                <option value="<?= htmlspecialchars($class['classID'], ENT_QUOTES, 'UTF-8') ?>" <?= (string)$class['classID'] === (string)$selectedClass ? 'selected' : '' ?>>
                    ...
                    <div class="attendance-filter-section">
                        <h4>Document Status Filter</h4>
                        <div class="filter-group">
                            <label class="filter-label" for="document_status">Filter by Document Status</label>
                            <select name="document_status" id="document_status">
                                <option value="">All Document Statuses</option>
                                <option value="Verified" <?= $documentStatusFilter === 'Verified' ? 'selected' : '' ?>>Verified</option>
                                <option value="Approved" <?= $documentStatusFilter === 'Approved' ? 'selected' : '' ?>>Approved</option>
                                <option value="Pending" <?= $documentStatusFilter === 'Pending' ? 'selected' : '' ?>>Pending</option>
                                <option value="Declined" <?= $documentStatusFilter === 'Declined' ? 'selected' : '' ?>>Declined</option>
                                <option value="Missing" <?= $documentStatusFilter === 'Missing' ? 'selected' : '' ?>>Missing</option>
                            </select>
                        </div>
                        <div class="filter-group">
                            <label class="filter-label" for="signature_status">Filter by Signature Status</label>
                            <select name="signature_status" id="signature_status">
                                <option value="">All Signature Statuses</option>
                                <option value="complete" <?= $signatureStatusFilter === 'complete' ? 'selected' : '' ?>>Complete (All Signatures)</option>
                                <option value="incomplete" <?= $signatureStatusFilter === 'incomplete' ? 'selected' : '' ?>>Incomplete (Missing Signatures)</option>
                            </select>
                        </div>
                        <div class="filter-group">
                            <label class="filter-label" for="document_type">Filter by Document Type</label>
                            <select name="document_type" id="document_type">
                                <option value="">All Document Types</option>
                                <option value="Learner Agreement" <?= ($_GET['document_type'] ?? '') === 'Learner Agreement' ? 'selected' : '' ?>>📋 Learner Agreement</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="filter-group">
                        ...
                        <button type="button" id="downloadBulkBtn" onclick="downloadBulkAgreements()" style="background: #28a745; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer; margin-left: 10px;">📥 Download Bulk Agreements</button>
                        
                        <?php if ($documentStatusFilter === 'Verified'): ?>
                        <button type="button" id="downloadCSVBtn" onclick="generateSelectedExcel()" style="background: #17a2b8; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer; margin-left: 10px;" disabled>💰 Download Payment Excel</button>
                        <?php endif; ?>
                    </div>
                </form>
            </div>
            ...
            <div style="background: #fff3cd; padding: 20px; margin: 20px 0; border: 2px solid #ffc107; border-radius: 10px;">
                <h3 style="color: #856404; margin-bottom: 15px;">🔍 Debug Information</h3>
                
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                    <div style="background: #f8f9fa; padding: 15px; border-radius: 8px;">
                        <h4 style="color: #495057; margin-bottom: 10px;">Current Page Data</h4>
                        <p><strong>Report Data Count:</strong> <?= count($reportData) ?></p>
                        <p><strong>Total Learners:</strong> <?= $totalLearners ?></p>
                        <p><strong>Start Date:</strong> <?= $startDate ?></p>
                        <p><strong>End Date:</strong> <?= $endDate ?></p>
                        <p><strong>Document Status Filter:</strong> <?= $documentStatusFilter ?: 'None' ?></p>
                        <p><strong>Signature Status Filter:</strong> <?= $signatureStatusFilter ?: 'None' ?></p>
                        <p><strong>Attendance Filters:</strong> <?= !empty($attendanceFilters) ? implode(', ', $attendanceFilters) : 'None' ?></p>
                    </div>
                    
                    <div style="background: #e3f2fd; padding: 15px; border-radius: 8px;">
                        <h4 style="color: #1565c0; margin-bottom: 10px;">CSV Query Test</h4>
                        <?php
                        // Use the same date range as the main page (from start_date and end_date GET parameters)
                        $startDate_test = $startDate;  // Uses the same $startDate as the main page
                        $endDate_test = $endDate;      // Uses the same $endDate as the main page
                        ...
                        $csv_test_result = $conn->query($csv_test_sql);
                        $csv_count = 0;
                        if ($csv_test_result) {
                            $csv_row = $csv_test_result->fetch_assoc();
                            $csv_count = $csv_row['total_count'];
                        }
                        
                        // Test without attendance filter
                        $csv_test_sql_no_filter = "
                            SELECT COUNT(*) as total_count
                            FROM sites s
                            JOIN class c ON s.siteID = c.siteID
                            JOIN learnerdetails ld ON c.classID = ld.classID
                        ...
                        $csv_test_result_no_filter = $conn->query($csv_test_sql_no_filter);
                        $csv_count_no_filter = 0;
                        if ($csv_test_result_no_filter) {
                            $csv_row_no_filter = $csv_test_result_no_filter->fetch_assoc();
                            $csv_count_no_filter = $csv_row_no_filter['total_count'];
                        }
                        ?>
                        
                        <p><strong>Debug Date Range:</strong> <span style="background: #fff3cd; padding: 2px 6px; border-radius: 3px; font-family: monospace;"><?= $startDate_test ?></span> to <span style="background: #fff3cd; padding: 2px 6px; border-radius: 3px; font-family: monospace;"><?= $endDate_test ?></span></p>
                        <p><strong>CSV Query (NO attendance filter):</strong> <?= $csv_count ?> learners</p>
                        <p><strong>Baseline Query (same as above):</strong> <?= $csv_count_no_filter ?> learners</p>
                        <p style="color: #28a745;"><strong>✅ All learners included regardless of attendance</strong></p>
                        <p style="font-size: 12px; color: #666; margin-top: 10px;">
                            <strong>To change date range:</strong> Add <code>&start_date=YYYY-MM-DD&end_date=YYYY-MM-DD</code> to URL<br>
                            <em>This uses the same date inputs as the main page form</em>
                        </p>
                        
                        <?php
                        // Test specific learners that should be included - ALL MISSING LEARNERS
                        $test_ids = [
                            '8807125044083', '9203036931088', '9306110515089', '9803201107081', '9210160312084',
                            '8303020438081', '9704201065081', '7412261196083', '8903190262086', '8008180388089',
                            '7305051262083', '8501215903088', '8201013311085', '9312101196087', '8502221370080',
                            '8104040195085', '8406220569082', '9510075528082', '8803301176086', '8005230418080',
                            '9504240219081', '7207130889086', '9409140975081', '411041213082', '8506281152080',
                            '9010090779083', '8405290720088', '209055549080', '8802150764083', '7011030315089',
                            '8808110572086', '8410101823089', '9811025950081', '411230632084', '9703020349080',
                            '8004276335084', '11015673087', '7607110516080', '9601105902080', '8908251177081',
                            '7703140673080', '606140984083', '8204170379085'
                        ];
                        $test_ids_string = "'" . implode("','", $test_ids) . "'";
                        
                        // Check if these learners exist in base query
                        $base_check_sql = "
                            SELECT ld.LearnerID, ld.Name, ld.Surname, ld.IDNumber, ld.activity_statu
                            FROM sites s
                            JOIN class c ON s.siteID = c.siteID
                            JOIN learnerdetails ld ON c.classID = ld.classID
                            WHERE ld.IDNumber IN ($test_ids_string)
                            AND (ld.activity_statu = '' OR ld.activity_statu IS NULL)
                        ";
                        
                        $base_check_result = $conn->query($base_check_sql);
                        echo "<p><strong>Test Learners in Base Query:</strong> " . ($base_check_result ? $base_check_result->num_rows : 0) . " found</p>";
                        
                        // Check with attendance filter
                        $attendance_check_sql = "
                            SELECT ld.LearnerID, ld.Name, ld.Surname, ld.IDNumber, 
                                   COALESCE(attendance.days_present, 0) as days_present
                            FROM sites s
                            JOIN class c ON s.siteID = c.siteID
                            JOIN learnerdetails ld ON c.classID = ld.classID
                            LEFT JOIN (
                                SELECT 
                                    ...
                                    COUNT(DISTINCT DATE(lc.clock_date)) as days_present
                                FROM learner_clocking lc
                                WHERE DATE(lc.clock_date) BETWEEN '{$startDate_test}' AND '{$endDate_test}'
                                GROUP BY lc.learnerID
                            ) attendance ON attendance.learnerID = ld.learnerID
                            WHERE ld.IDNumber IN ($test_ids_string)
                            AND (ld.activity_statu = '' OR ld.activity_statu IS NULL)
                        ...
                        if ($attendance_check_result) {
                            echo "<p><strong>Test Learners with Attendance Data:</strong></p>";
                            echo "<ul>";
                            while ($row = $attendance_check_result->fetch_assoc()) {
                                $filtered_out = ($row['days_present'] == 0) ? " (FILTERED OUT - 0 attendance)" : " (INCLUDED)";
                                echo "<li>{$row['Name']} {$row['Surname']} (ID: {$row['IDNumber']}) - {$row['days_present']} days{$filtered_out}</li>";
                            }
                            echo "</ul>";
                        }
                        ?>
                ...
                <div style="background: #fff3cd; padding: 15px; border-radius: 8px; margin-bottom: 20px;">
                    <h4 style="color: #856404; margin-bottom: 10px;">🔍 Missing Learners Analysis (43 learners)</h4>
                    <?php
                    // Check why specific learners are missing
                    $missing_analysis_sql = "
                        SELECT 
                            ...
                            ld.IDNumber,
                            ld.activity_statu,
                            COALESCE(bd.BankName, 'NO BANK') as bank_status,
                            COALESCE(ld_doc.status, 'NO DOC') as doc_status,
                            COALESCE(attendance.days_present, 0) as attendance_days,
                            s.siteName,
                            s.project_id,
                            'FOUND' as status
                        FROM sites s
                        ...
                        WHERE ld.IDNumber IN ($test_ids_string)
                        ORDER BY ld.IDNumber
                    ";
                    
                    $missing_result = $conn->query($missing_analysis_sql);
                    $found_learners = [];
                    $found_count = 0;
                    $zero_attendance_count = 0;
                    $has_attendance_count = 0;
                    ...
                    if ($missing_result) {
                        while ($row = $missing_result->fetch_assoc()) {
                            ...
                            if ($row['attendance_days'] == 0) {
                                ...
                                $has_attendance_count++;
                            }
                        }
                    }
                    
                    $not_found_count = count($test_ids) - $found_count;
                    
                    echo "<div style='display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap: 10px; margin-bottom: 15px;'>";
                    echo "<div style='background: #e3f2fd; padding: 10px; border-radius: 5px; text-align: center;'>";
                    echo "<strong>Total Checked</strong><br><span style='font-size: 24px; color: #1565c0;'>" . count($test_ids) . "</span>";
                    echo "</div>";
                    echo "<div style='background: #e8f5e8; padding: 10px; border-radius: 5px; text-align: center;'>";
                    echo "<strong>Found in DB</strong><br><span style='font-size: 24px; color: #2e7d2e;'>{$found_count}</span>";
                    echo "</div>";
                    echo "<div style='background: #ffebee; padding: 10px; border-radius: 5px; text-align: center;'>";
                    echo "<strong>Not Found</strong><br><span style='font-size: 24px; color: #c62828;'>{$not_found_count}</span>";
                    echo "</div>";
                    echo "<div style='background: #fff3e0; padding: 10px; border-radius: 5px; text-align: center;'>";
                    echo "<strong>Zero Attendance</strong><br><span style='font-size: 24px; color: #f57c00;'>{$zero_attendance_count}</span>";
                    echo "</div>";
                    echo "</div>";
                    ...
                    if ($found_count > 0) {
                        ...
                        echo "<tr style='background: #ffeaa7; position: sticky; top: 0;'>";
                        echo "<th style='border: 1px solid #ddd; padding: 6px;'>ID Number</th>";
                        echo "<th style='border: 1px solid #ddd; padding: 6px;'>Name</th>";
                        echo "<th style='border: 1px solid #ddd; padding: 6px;'>Activity Status</th>";
                        echo "<th style='border: 1px solid #ddd; padding: 6px;'>Bank</th>";
                        echo "<th style='border: 1px solid #ddd; padding: 6px;'>Doc</th>";
                        echo "<th style='border: 1px solid #ddd; padding: 6px;'>Attendance</th>";
                        echo "<th style='border: 1px solid #ddd; padding: 6px;'>Site</th>";
                        echo "<th style='border: 1px solid #ddd; padding: 6px;'>Project</th>";
                        echo "</tr>";
                        
                        foreach ($found_learners as $row) {
                            $row_color = ($row['attendance_days'] == 0) ? 'background: #ffcccb;' : 'background: #d4edda;';
                            echo "<tr style='{$row_color}'>";
                            echo "<td style='border: 1px solid #ddd; padding: 4px; font-family: monospace;'>{$row['IDNumber']}</td>";
                            echo "<td style='border: 1px solid #ddd; padding: 4px;'>{$row['Name']} {$row['Surname']}</td>";
                            echo "<td style='border: 1px solid #ddd; padding: 4px;'>" . ($row['activity_statu'] ?: 'EMPTY') . "</td>";
                            echo "<td style='border: 1px solid #ddd; padding: 4px;'>{$row['bank_status']}</td>";
                            echo "<td style='border: 1px solid #ddd; padding: 4px;'>{$row['doc_status']}</td>";
                            echo "<td style='border: 1px solid #ddd; padding: 4px; text-align: center;'><strong>{$row['attendance_days']}</strong></td>";
                            echo "<td style='border: 1px solid #ddd; padding: 4px; font-size: 10px;'>{$row['siteName']}</td>";
                            echo "<td style='border: 1px solid #ddd; padding: 4px; text-align: center;'>{$row['project_id']}</td>";
                            echo "</tr>";
                        }
                        echo "</table>";
                        echo "</div>";
                        echo "<p style='margin-top: 10px; font-size: 12px;'>";
                        echo "<span style='background: #ffcccb; padding: 2px 5px;'>Red = 0 attendance</span> ";
                        echo "<span style='background: #d4edda; padding: 2px 5px;'>Green = Has attendance</span>";
                        echo "</p>";
                    }
                    
                    // Show which IDs were not found
                    if ($not_found_count > 0) {
                        $found_id_numbers = array_column($found_learners, 'IDNumber');
                        $not_found_ids = array_diff($test_ids, $found_id_numbers);
                        echo "<div style='background: #ffebee; padding: 10px; border-radius: 5px; margin-top: 15px;'>";
                        echo "<h5 style='color: #c62828; margin-bottom: 10px;'>❌ Learners NOT FOUND in Database ({$not_found_count}):</h5>";
                        echo "<div style='font-family: monospace; font-size: 11px; column-count: 3; column-gap: 20px;'>";
                        foreach ($not_found_ids as $id) {
                            echo "<div style='break-inside: avoid; padding: 2px;'>{$id}</div>";
                        }
                        echo "</div>";
                        echo "</div>";
                    }
                ...
                <div style="background: #f8f9fa; padding: 15px; border-radius: 8px; margin-top: 15px;">
                    <h4 style="color: #495057; margin-bottom: 10px;">Sample Learners (First 10)</h4>
                    <table style="width: 100%; border-collapse: collapse; font-size: 12px;">
                        <tr style="background: #e9ecef;">
                            <th style="border: 1px solid #dee2e6; padding: 8px;">ID</th>
                            <th style="border: 1px solid #dee2e6; padding: 8px;">Name</th>
                            <th style="border: 1px solid #dee2e6; padding: 8px;">Surname</th>
                            <th style="border: 1px solid #dee2e6; padding: 8px;">ID Number</th>
                            <th style="border: 1px solid #dee2e6; padding: 8px;">Attendance %</th>
                        </tr>
                        <?php 
                        $count = 0;
                        foreach ($reportData as $learnerID => $learner): 
                            if ($count >= 10) break;
                            $attendancePercent = 0;
                            if (isset($learner['attendance_data'])) {
                                $totalPresent = array_sum($learner['attendance_data']);
                                $attendancePercent = count($dates) > 0 ? round(($totalPresent / count($dates)) * 100, 1) : 0;
                            }
                        ?>
                        <tr>
                            <td style="border: 1px solid #dee2e6; padding: 8px;"><?= htmlspecialchars($learnerID) ?></td>
                            <td style="border: 1px solid #dee2e6; padding: 8px;"><?= htmlspecialchars($learner['name']) ?></td>
                            <td style="border: 1px solid #dee2e6; padding: 8px;"><?= htmlspecialchars($learner['surname']) ?></td>
                            <td style="border: 1px solid #dee2e6; padding: 8px;"><?= htmlspecialchars($learner['id_number']) ?></td>
                            <td style="border: 1px solid #dee2e6; padding: 8px;"><?= $attendancePercent ?>%</td>
                        </tr>
                        <?php 
                            $count++;
                        endforeach; 
                ...
                <div style="margin-top: 15px; font-size: 14px; color: #6c757d;">
                    <p><strong>To access CSV debug:</strong> Add <code>&debug_csv=1</code> to your CSV download URL</p>
                    <p><strong>Goal:</strong> Show learners with attendance > 0% (currently showing <?= count($reportData) ?>)</p>
                    <p style="color: #dc3545;"><strong>❌ Zero attendance filter ENABLED - excludes learners with 0% attendance</strong></p>
                </div>
            </div>
            <?php endif; ?>
            ...
            <!-- <div style="background: #f8f9fa; padding: 10px; margin: 10px 0; border: 1px solid #dee2e6; border-radius: 5px;">
                ...
                Report Data Count: <?= count($reportData) ?><br>
                Total Learners: <?= $totalLearners ?><br>
                Start Date: <?= $startDate ?><br>
                End Date: <?= $endDate ?><br>
                Document Status Filter: <?= $documentStatusFilter ?><br>
                Selected District: <?= $selectedDistrict ?><br>
                Selected Site: <?= $selectedSite ?><br>
                Attendance Filters: <?= implode(', ', $attendanceFilters) ?><br>
                <small>Check error logs for detailed SQL query information.</small>
            </div> -->
            ...
            <?php if (!empty($reportData)): ?>
                <div class="report-section">
                    ...
                    <?php 
                    // Count learners with complete signatures
                    $completeSignatureCount = 0;
                    foreach ($reportData as $learner) {
                        if (!empty($learner['signature']) && 
                            !empty($learner['learner_initials']) && 
                            !empty($learner['witness_signature']) && 
                            !empty($learner['witness_initials'])) {
                            $completeSignatureCount++;
                        }
                    }
                    ?>
                    <div style="background: #e8f5e8; border: 1px solid #28a745; border-radius: 5px; padding: 10px; margin: 10px 0; text-align: center;">
                        <strong>📋 Auto-Selection Status:</strong> 
                        <span style="color: #28a745; font-weight: bold;"><?= $completeSignatureCount ?></span> learners with complete signatures are automatically selected for bulk download
                    </div>
                        ...
                        <table>
                            <thead>
                                <tr>
                                    ...
                                    <th rowspan="2" style="vertical-align: middle;">📋 Learner Agreement</th>
                                    <th rowspan="2" style="vertical-align: middle;">✍️ Signature</th>
                                    <th rowspan="2" style="vertical-align: middle;">📝 Learner Initials</th>
                                    <th rowspan="2" style="vertical-align: middle;">👥 Witness Signature</th>
                                    <th rowspan="2" style="vertical-align: middle;">📝 Witness Initials</th>
                                    <th rowspan="2" style="vertical-align: middle; min-width: 100px; white-space: normal;">📥 Download<br>Agreement</th>
                                ...
                                <?php foreach ($reportData as $learnerID => $learner): ?>
                                    ...
                                    <tr class="learner-row">
                                        <td class="learner-details">
                                            <?php 
                                            // Check if all signature fields are completed
                                            $allSignaturesComplete = !empty($learner['signature']) && 
                                                                   !empty($learner['learner_initials']) && 
                                                                   !empty($learner['witness_signature']) && 
                                                                   !empty($learner['witness_initials']);
                                            ?>
                                            <?php if ($allSignaturesComplete): ?>
                                                <input type="checkbox" class="learner-checkbox" value="<?= htmlspecialchars($learner['id_number'], ENT_QUOTES, 'UTF-8') ?>" 
                                                       data-name="<?= htmlspecialchars($learner['surname'] . ' ' . $learner['name'], ENT_QUOTES, 'UTF-8') ?>"
                                                       data-amount="<?= $amountDue ?>" checked>
                                            <?php else: ?>
                                                <span style="color: #999; font-size: 0.8em;">Incomplete</span>
                                            <?php endif; ?>
                                        </td>
                                        ...
                                        <?php 
                                        $docStatuses = [
                                            'learner_agreement_status'
                                        ];
                                        foreach ($docStatuses as $docStatus):
                                            $status = isset($learner[$docStatus]) ? $learner[$docStatus] : 'Missing';
                                            $statusClass = strtolower(str_replace('/', '', $status));
                                        ?>
                                        <td class="document-status status-<?= $statusClass ?>"><?= htmlspecialchars($status) ?></td>
                                        <?php endforeach; ?>
                                        
                                        <!-- Signature Fields -->
                                        <td class="signature-field">
                                            <?php $learnerSigPath = detectValidSignature($conn, $learnerID); ?>
                                            <?php if (!empty($learnerSigPath)): ?>
                                                <img src='<?= htmlspecialchars($learnerSigPath, ENT_QUOTES, 'UTF-8') ?>' 
                                                     class='signature-img rounded' 
                                                     alt='Learner Signature' 
                                                     style='width: 100px; height: 50px; border: 1px solid #ddd; border-radius: 4px;' 
                                                     onerror="this.parentNode.innerHTML='\u2717 Not Signed';">
                                            <?php else: ?>
                                                <span class="signature-status unsigned">✗ Not Signed</span>
                                            <?php endif; ?>
                                        </td>
                                        <td class="initials-field"><?= htmlspecialchars($learner['learner_initials'] ?? 'N/A', ENT_QUOTES, 'UTF-8') ?></td>
                                        <td class="signature-field">
                                            <?php $webWitnessSignaturePath = detectValidWitnessSignature($conn, $learnerID); ?>
                                            <?php if (!empty($webWitnessSignaturePath)): ?>
                                                <img src='<?= htmlspecialchars($webWitnessSignaturePath, ENT_QUOTES, 'UTF-8') ?>' 
                                                     class='signature-img rounded' 
                                        ...
                                        <td class="initials-field"><?= htmlspecialchars($learner['witness_initials'] ?? 'N/A', ENT_QUOTES, 'UTF-8') ?></td>
                                         <!-- Download Agreement Button -->
                                        <td style="text-align: center;">
                                            <a href="mobile/new_aggrement22.php?LearnerID=<?= htmlspecialchars($learnerID, ENT_QUOTES, 'UTF-8') ?>&force_zip=1" 
                                               class="btn btn-sm btn-primary" 
                                               style="padding: 5px 10px; font-size: 12px; text-decoration: none; display: inline-block; background-color: #007bff; color: white; border-radius: 4px;">
                                                📥 Download
                                            </a>
                                        </td>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Add event listeners for document status filter and date changes
            const documentStatusSelect = document.getElementById('document_status');
            const startDateInput = document.getElementById('start_date');
            const endDateInput = document.getElementById('end_date');
            
            if (documentStatusSelect) {
                documentStatusSelect.addEventListener('change', checkSickNotesAndUpdateButton);
            }
            if (startDateInput) {
                startDateInput.addEventListener('change', checkSickNotesAndUpdateButton);
            }
            if (endDateInput) {
                endDateInput.addEventListener('change', checkSickNotesAndUpdateButton);
            }
            
            // Initial check when page loads
            checkSickNotesAndUpdateButton();
        });

        // Function to check for pending sick notes and disable download button
        function checkSickNotesAndUpdateButton() {
            const documentStatusFilter = document.getElementById('document_status').value;
            const downloadButton = document.querySelector('button[onclick="downloadPaymentCSV()"]');
            
            // Check if downloadButton exists before proceeding
            if (!downloadButton) {
                // Skipping sick notes check - no download button
                return;
            }
            
            if (documentStatusFilter !== 'Verified') {
                // Check for pending sick notes via AJAX
                ...
                const projectId = formData.get('project_id') || '1';
                
                // Make AJAX request to check for pending sick notes
                fetch(`?check_sick_notes=1&project_id=${projectId}&year=${year}&month=${month}`)
                    .then(response => response.json())
                    .then(data => {
                        if (data.has_pending_sick_notes) {
                            downloadButton.disabled = true;
                            downloadButton.title = 'Cannot download: There are learners with pending sick notes. Please filter by "Verified" status only.';
                            downloadButton.style.opacity = '0.5';
                            downloadButton.style.cursor = 'not-allowed';
                        } else {
                            downloadButton.disabled = false;
                            downloadButton.title = 'Download Payment CSV';
                            downloadButton.style.opacity = '1';
                            downloadButton.style.cursor = 'pointer';
                        }
                    })
                    .catch(error => {
                        console.error('Error checking sick notes:', error);
                        // If error, enable button but show warning
                        downloadButton.disabled = false;
                        downloadButton.title = 'Download Payment CSV (Unable to verify sick notes status)';
                    });
            } else {
                // If filtering by Verified, always enable the button
                downloadButton.disabled = false;
                downloadButton.title = 'Download Payment CSV (Verified learners only)';
                downloadButton.style.opacity = '1';
                downloadButton.style.cursor = 'pointer';
            }
        }

        // Function to download Payment CSV
        function downloadPaymentCSV() {
            ...
            if (downloadButton.disabled) {
                alert('Cannot download: There are learners with pending sick notes. Please filter by "Verified" status only or resolve pending sick notes first.');
                return;
            }
            ...
            const startDateObj = new Date(startDate);
            const year = startDateObj.getFullYear();
            const month = startDateObj.getMonth() + 1; // JavaScript months are 0-based
            
            // Get project_id from the form or use a default
            const projectId = formData.get('project_id') || '1'; // Default to 1 if not set
            
            // Build the download URL
            const downloadUrl = `?download_payment_csv=1&project_id=${projectId}&year=${year}&month=${month}`;
            
            // Add current filter parameters to maintain context
            const district = formData.get('district');
            const site = formData.get('site');
            const idSearch = formData.get('id_search');
            const documentStatus = formData.get('document_status');
            
            let urlParams = downloadUrl;
            if (district) urlParams += `&district=${encodeURIComponent(district)}`;
            if (site) urlParams += `&site=${encodeURIComponent(site)}`;
            if (idSearch) urlParams += `&id_search=${encodeURIComponent(idSearch)}`;
            if (documentStatus) urlParams += `&document_status=${encodeURIComponent(documentStatus)}`;
        ...
        // Function to validate DR account fields and enable/disable download button
        function validateDRFields() {
            // Check if elements exist first
            const drAccountNameEl = document.getElementById('dr_account_name');
            const drAccountNumberEl = document.getElementById('dr_account_number');
            const drBranchNumberEl = document.getElementById('dr_branch_number');
            const downloadBtn = document.getElementById('downloadCSVBtn');
            
            // Only proceed if all elements exist
            if (!drAccountNameEl || !drAccountNumberEl || !drBranchNumberEl) {
                return; // Elements don't exist, skip validation
            }
            
            const drAccountName = drAccountNameEl.value.trim();
            const drAccountNumber = drAccountNumberEl.value.trim();
            const drBranchNumber = drBranchNumberEl.value.trim();
            
            if (downloadBtn) {
                // Check if all three fields have values
                if (drAccountName && drAccountNumber && drBranchNumber) {
                    downloadBtn.disabled = false;
                    downloadBtn.style.opacity = '1';
                    downloadBtn.style.cursor = 'pointer';
                    downloadBtn.title = 'Download Payment Excel';
                } else {
                    downloadBtn.disabled = true;
                    downloadBtn.style.opacity = '0.5';
                    downloadBtn.style.cursor = 'not-allowed';
                    downloadBtn.title = 'Please fill in all DR account fields (Name, Account Number, Branch Number)';
                }
            }
        }
        ...
        function updateSelectAllState() {
            ...
            if (learnerCheckboxes.length === 0) return;
            
            const checkedCount = document.querySelectorAll('.learner-checkbox:checked').length;
            const totalCount = learnerCheckboxes.length;
        ...
        // Function to generate Excel for selected learners
        function generateSelectedExcel() {
            // Check if download button is disabled (DR fields validation)
            const downloadBtn = document.getElementById('downloadCSVBtn');
            if (downloadBtn && downloadBtn.disabled) {
                alert('Please fill in all DR account fields (Name, Account Number, Branch Number) before downloading Excel file.');
                return;
            }
            ...
            if (selectedCheckboxes.length === 0) {
                alert('Please select at least one learner to generate Excel file.');
                return;
            }
            
            // Collect selected learner data
            const selectedLearners = [];
            selectedCheckboxes.forEach(checkbox => {
                selectedLearners.push({
                    id_number: checkbox.value,
            ...
            // Get project_id from the form or use a default
            const projectId = formData.get('project_id') || '76'; // Default to 76 if not set
            
            // Create a form for POST submission
            const postForm = document.createElement('form');
            postForm.method = 'POST';
            postForm.action = window.location.pathname;
            postForm.style.display = 'none';
            
            // Get DR account details from the form
            const drAccountName = document.getElementById('dr_account_name').value || '';
            const drAccountNumber = document.getElementById('dr_account_number').value || '';
            const drBranchNumber = document.getElementById('dr_branch_number').value || '';
            
            // Add all necessary parameters
            const params = {
                'export_selected_excel': '1',
                'project_id': projectId,
                'year': year,
                'month': month,
                'selected_learners': JSON.stringify(selectedLearners),
                'district': formData.get('district') || '',
                'site': formData.get('site') || '',
                'id_search': formData.get('id_search') || '',
                'document_status': formData.get('document_status') || '',
                'signature_status': formData.get('signature_status') || '',
                'start_date': startDate,
                'end_date': endDate,
                'dr_account_name': drAccountName,
                'dr_account_number': drAccountNumber,
                'dr_branch_number': drBranchNumber
            };
            ...
            // Clear the DR account fields after export
            document.getElementById('dr_account_name').value = '';
            document.getElementById('dr_account_number').value = '';
            document.getElementById('dr_branch_number').value = '';
        }

        // Function to download bulk agreements for selected learners
        function downloadBulkAgreements() {
            const selectedCheckboxes = document.querySelectorAll('.learner-checkbox:checked');
            
            if (selectedCheckboxes.length === 0) {
                alert('Please select at least one learner with completed signatures to download bulk agreements.');
                return;
            }
            
            // Collect selected learner ID numbers
            const selectedIdNumbers = [];
            selectedCheckboxes.forEach(checkbox => {
                selectedIdNumbers.push(checkbox.value);
            });
            
            // Check if we need to use chunked processing for large downloads
            if (selectedIdNumbers.length > 10) {
                // Use direct chunked processing for large downloads (avoids timeout issues)
                startDirectChunkedDownload(selectedIdNumbers);
                return;
            }
            ...
            // Create a form for POST submission to new_aggrement_Bulk.php (avoid URL length issues)
            const postForm = document.createElement('form');
            postForm.method = 'POST';
            postForm.action = 'mobile/new_aggrement_Bulk.php';
            postForm.style.display = 'none';
            
            // Add all necessary parameters
            const params = {
                'IDNumbers': selectedIdNumbers.join(','),
                'district': formData.get('district') || '',
                'site': formData.get('site') || '',
                'start_date': startDate,
                'end_date': endDate,
                'project_id': formData.get('project_id') || '76'
            };
        ...
        // Direct chunked download function for large numbers of learners (improved version)
        function startDirectChunkedDownload(selectedIdNumbers) {
            // Show processing modal
            showBulkDownloadModal(selectedIdNumbers.length);
            
            // Get current form values for filters
            const form = document.querySelector('form');
            const formData = new FormData(form);
            
            // Create a form for direct submission to improved chunked processor
            const postForm = document.createElement('form');
            postForm.method = 'POST';
            postForm.action = 'disk_space_optimized_bulk_processor.php';
            postForm.target = 'chunkedDownload';
            postForm.style.display = 'none';
            ...
            // Open in new window with larger size for better visibility
            window.open('', 'chunkedDownload', 'width=1200,height=800,scrollbars=yes,resizable=yes');
        ...
        // Function to use existing system for smaller downloads
        function downloadBulkAgreementsExisting(selectedIdNumbers, filters) {
            // Create a form for POST submission to new_aggrement_Bulk.php
            const postForm = document.createElement('form');
            postForm.method = 'POST';
            postForm.action = 'mobile/new_aggrement_Bulk.php';
            postForm.style.display = 'none';
        ...
        // Show modal for bulk download processing
        function showBulkDownloadModal(learnerCount) {
            // Create modal if it doesn't exist
            let modal = document.getElementById('bulkDownloadModal');
            if (!modal) {
                ...
                modal.innerHTML = `
                    <div style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.7); z-index: 10000; display: flex; align-items: center; justify-content: center;">
                        <div style="background: white; padding: 30px; border-radius: 10px; text-align: center; max-width: 500px; box-shadow: 0 4px 20px rgba(0,0,0,0.3);">
                            <h3 style="color: #2c3e50; margin-bottom: 20px;">🚀 Enhanced Bulk Download</h3>
                            <p style="color: #7f8c8d; margin-bottom: 20px;">Processing <strong>${learnerCount}</strong> learners using chunked processing...</p>
                            <div style="width: 100%; height: 6px; background: #ecf0f1; border-radius: 3px; overflow: hidden; margin-bottom: 20px;">
                                <div style="width: 30%; height: 100%; background: linear-gradient(90deg, #3498db, #2ecc71); animation: loading 2s infinite;"></div>
                            </div>
                            <p style="color: #95a5a6; font-size: 14px;">Opening enhanced download system...</p>
                        </div>
                    </div>
                    <style>
                        @keyframes loading {
        ...
        // Hide modal
        function hideBulkDownloadModal() {
        ...
        function populateProjectDropdown() {
            const projectSelect = document.getElementById('selected_project');
        ...
        async function handleProjectChange() {
            ...
            // Update hidden project_id field
            projectIdInput.value = selectedProjectId;
            
            // Clear dependent dropdowns
            districtSelect.innerHTML = '<option value="">-- All Districts --</option>';
            if (siteSelect) siteSelect.innerHTML = '<option value="">-- All Sites --</option>';
            if (classSelect) classSelect.innerHTML = '<option value="">-- All Classes --</option>';
            
            if (selectedProjectId) {
                try {
                    // Load districts for selected project
                    const response = await fetch(`get_districts_by_project.php?project_id=${selectedProjectId}`);
                    const data = await response.json();
                    ...
                    if (data.success) {
                        ...
                        console.log(`Loaded ${data.districts.length} districts for project ${selectedProjectId}`);
                    } else {
                        console.error('Failed to load districts:', data.error);
                        alert('Failed to load districts for selected project');
                    }
                } catch (error) {
            ...
            // DO NOT submit form - let user select district/site/class first
            // Form will be submitted when Apply Filters button is clicked
        }
        
        // Handle district change - load sites
        async function handleDistrictChange() {
            ...
            if (selectedProjectId && selectedDistrict) {
                try {
                    // Load sites for selected project and district
                    const response = await fetch(`get_sites_by_district.php?project_id=${selectedProjectId}&district=${encodeURIComponent(selectedDistrict)}`);
                    const data = await response.json();
                    ...
                    if (data.success) {
                        ...
                        console.log(`Loaded ${data.sites.length} sites for district ${selectedDistrict}`);
                    } else {
        ...
        // Handle site change - load classes
        async function handleSiteChange() {
            ...
            if (selectedProjectId && selectedSite) {
                try {
                    // Load classes for selected site
                    const response = await fetch(`get_classes_by_site.php?project_id=${selectedProjectId}&site=${encodeURIComponent(selectedSite)}`);
                    const data = await response.json();
                    ...
                    if (data.success) {
                        ...
                        console.log(`Loaded ${data.classes.length} classes for site ${selectedSite}`);
                    } else {
        ...
        // Initialize project dropdown when page loads
        document.addEventListener('DOMContentLoaded', function() {
            loadProjects();
            
            // Add event listeners for district and site dropdowns
            const districtSelect = document.getElementById('district');
            const siteSelect = document.getElementById('site');
