    ...
    if (count($learners) === 1) {
        if (count($generatedFiles) === 1 && !$forceZip) {
            ...
            try {
                ...
                // Convert DOCX files to PDF before adding to ZIP
                $filesToAdd = [];
                $docxFilesToCleanup = [];
                ...
                foreach ($generatedFiles as $file) {
                    if (file_exists($file)) {
                        ...
                        if ($fileExtension === 'docx') {
                            log_message("Converting to PDF: $file");
                            $pdfPath = convertDocxToPdfLibreOffice($file, $output_dir);
                            
                            if ($pdfPath && file_exists($pdfPath)) {
                                log_message("PDF conversion successful: $pdfPath");
                                $filesToAdd[] = $pdfPath;
                                $docxFilesToCleanup[] = $file; // Mark DOCX for cleanup
                            } else {
                                log_message("PDF conversion failed for: $file - adding DOCX instead");
                                $filesToAdd[] = $file; // Fallback to DOCX if PDF conversion fails
                            }
                        } else {
                            // For non-DOCX files, add as-is
                            $filesToAdd[] = $file;
                        }
                    }
                }
                
                // Add converted/original files to ZIP
        ...
        try {
            ...
            $filesToAdd = [];
            $docxFilesToCleanup = [];
            $storedPdfInfo = []; // Track stored PDFs for database
                ...
                if (file_exists($file)) {
                    ...
                    if ($fileExtension === 'docx') {
                        ...
                        if ($pdfPath && file_exists($pdfPath)) {
                            ...
                            // Extract learner ID and document type from filename
                            $filename = basename($pdfPath);
                            if (preg_match('/^(\d+)_(.+)\.pdf$/', $filename, $matches)) {
                                $learnerIDForStorage = $matches[1];
                                $documentType = str_replace('_', ' ', $matches[2]); // e.g., "Agreement" or "POE Form"
                                
                                $storedPdfInfo[] = [
                                    'pdf_path' => $pdfPath,
                                    'learner_id' => $learnerIDForStorage,
                                    'filename' => $filename,
            ...
            // Add converted/original files to ZIP (organized by learner folder)
            foreach ($filesToAdd as $file) {
                if (file_exists($file)) {
                    $filename = basename($file);
                    
                    // Extract learner ID from filename (e.g., "7704071056089_Agreement.pdf" -> "7704071056089")
                    if (preg_match('/^(\d+)_/', $filename, $matches)) {
                        $learnerID = $matches[1];
                        $zipPath = $learnerID . '/' . $filename;
                    } else {
                        // Fallback: use filename only if pattern doesn't match
                        $zipPath = $filename;
                    }
                    
                    $zip->addFile($file, $zipPath);
                    log_message("Added to ZIP: " . $zipPath);
                } else {
                    log_message("ZIP error: Converted file not found: $file");
                }
            }
            ...
            if (file_exists($zipFileName)) {
                log_message("ZIP file created: $zipFileName (with " . count($filesToAdd) . " PDF files)");
                
                // Store PDFs permanently on server for viewing
                if (!empty($storedPdfInfo)) {
                    log_message("Storing " . count($storedPdfInfo) . " PDFs permanently on server...");
                    
                    foreach ($storedPdfInfo as $pdfInfo) {
                        // Get learner details from database using correct table structure
                        ...
                        if ($learnerRow = $learnerResult->fetch_assoc()) {
                            ...
                            // Get document type from filename or use from pdfInfo
                    ...
                    log_message("PDF storage complete - " . count($storedPdfInfo) . " PDFs available for viewing on server");
                }
...
if (!empty($generatedFiles)) {
    if (count($learners) === 1) {
        ...
        if (count($generatedFiles) === 1) {
            ...
            // Convert DOCX to PDF before serving
            $fileExtension = strtolower(pathinfo($docxFile, PATHINFO_EXTENSION));
            
            if ($fileExtension === 'docx') {
                log_message("Converting single file to PDF: $docxFile");
                $pdfPath = convertDocxToPdfLibreOffice($docxFile, $output_dir);
                ...
                if ($pdfPath && file_exists($pdfPath)) {
                    ...
                    log_message("PDF conversion failed - falling back to DOCX: $docxFile");
                    // Fallback to serving DOCX if PDF conversion fails
                    header('Content-Description: File Transfer');
                    header('Content-Type: application/vnd.openxmlformats-officedocument.wordprocessingml.document');
                    header('Content-Disposition: attachment; filename="' . basename($docxFile) . '"');
                    header('Content-Transfer-Encoding: binary');
            ...
            try {
                ...
                if (file_exists($zipFileName)) {
                    log_message("ZIP file created: $zipFileName (with " . count($filesToAdd) . " files, including PDFs)");
