    ...
    while ($row = $result->fetch_assoc()) {
        ...
        foreach ($uniqueUnitStandards as $unitStandard) {
            ...
            if ($row['unit_standard_id'] && (string)$row['unit_standard_id'] === (string)$unitStandardId) {
                ...
                // Determine the correct category based on POE type
                ...
                } elseif ($assessmentType === 'logbook') {
                    $categoryType = 'logbook';
                }
                ...
                if (in_array($categoryType, ['formative', 'summative', 'logbook', 'formativeremedial', 'summativeremedial'])) {
                    $assessment = [
                    ...
                    file_put_contents('debug.log', "Assessment data (category: $categoryType): " . print_r($assessment, true) . "\n", FILE_APPEND);
                    $data['pathways'][$pathwayName]['qualifications'][$qualificationName]['unitstandards'][$unitStandardName][$categoryType][] = $assessment;
                } else {
                    file_put_contents('debug.log', "Invalid category_type: $categoryType (assessment_type: $assessmentType, poe_type: {$row['poe_type']})\n", FILE_APPEND);
                }
            }
        }
    }
