...
try {
    ...
    if ($result->num_rows > 0) {
        ...
        if ($isUpdate) {
            ...
            $updateQuery = $conn->prepare("UPDATE marks SET marks_scored = ?, a_comment = ? WHERE id = ?");
            if (!$updateQuery) {
            ...
            $updateQuery->bind_param("isi", $marksScored, $aComment, $recordId);
    ...
    $stmt = $conn->prepare("INSERT INTO marks (learnerID, exercise, so, marks_scored, type, a_comment) VALUES (?, ?, ?, ?, ?, ?)");
    if (!$stmt) {
        throw new Exception("Insert prepare failed: " . $conn->error);
    }

    // Bind parameters with correct types
    error_log("Binding values - learnerID: $learnerId, exercise: $exerciseName, so: $specificOutcomeStr, marks_scored: $marksScored, type: $actualAssessmentType, a_comment: $aComment");
    $stmt->bind_param("ississ", $learnerId, $exerciseName, $specificOutcomeStr, $marksScored, $actualAssessmentType, $aComment);
