...
for ($i = 0; $i < count($learners); $i += $batch_size) {
    ...
    foreach ($batch as $data) {
        ...
        try {
            ...
            // Copy ID Document and Qualifications to learner folder
            $document_names = ['ID Document', 'Qualifications'];
            foreach ($document_names as $doc_name) {
                log_message("Looking for $doc_name for LearnerID {$data['LearnerID']}");
                
                // Fetch the document from learner_document
                $sql_doc = "SELECT learner_document FROM learner_document WHERE learner_id = ? AND documentName = ? ORDER BY document_id DESC LIMIT 1";
                $stmt_doc = $conn->prepare($sql_doc);
                if ($stmt_doc) {
                    $stmt_doc->bind_param("is", $data['LearnerID'], $doc_name);
                    $stmt_doc->execute();
                    $result_doc = $stmt_doc->get_result();
                    $row_doc = $result_doc->fetch_assoc();
                    $stmt_doc->close();
                    
                    if ($row_doc && !empty($row_doc['learner_document'])) {
                        $documentPath = $row_doc['learner_document'];
                        
                        // Normalize document path (same as admin_document_view.php)
                        if (!str_starts_with($documentPath, 'learner_documents/')) {
                            $documentPath = 'learner_documents/' . ltrim($documentPath, '/');
                        }
                        $clean_filename = basename($documentPath);
                        $possible_paths = [];
                        
                        // If the database already contains a path prefix, try it first
                        if (strpos($documentPath, '/') !== false) {
                            $possible_paths[] = $documentPath;
                            $possible_paths[] = '../' . $documentPath; // Since we're in mobile/
                        }
                        
                        // Add comprehensive search paths like admin_document_view.php
                        $base_paths = [
                            'learner_documents/',
                            'mobile/learner_documents/',
                            'mobile/',
                            '',
                            '../learner_documents/',
                            '../mobile/learner_documents/',
                        ...
                        foreach ($base_paths as $base_path) {
                            ...
                            $possible_paths[] = $base_path . $clean_filename;
                            
                            // If filename doesn't end with common extensions, try adding them
                            if (!preg_match('/\.(pdf|png|jpg|jpeg|docx)$/i', $clean_filename)) {
                                $possible_paths[] = $base_path . $clean_filename . '.pdf';
                                $possible_paths[] = $base_path . $clean_filename . '.png';
                                $possible_paths[] = $base_path . $clean_filename . '.jpg';
                                $possible_paths[] = $base_path . $clean_filename . '.jpeg';
                                $possible_paths[] = $base_path . $clean_filename . '.docx';
                            }
                        }
                        
                        // Also try with the original database value (without normalization) just in case
                        $originalPath = $row_doc['learner_document'];
                        $possible_paths[] = $originalPath;
                        $possible_paths[] = '../' . $originalPath;
                        ...
                        if ($source_path) {
                            ...
                            $safe_doc_name = preg_replace('/[^a-zA-Z0-9]/', '_', $doc_name);
                            $file_ext = pathinfo($source_path, PATHINFO_EXTENSION);
                            if (empty($file_ext)) {
                                $file_ext = 'pdf';
                            }
                            $target_filename = $safe_id . '_' . $safe_doc_name . '.' . $file_ext;
                            $target_path = $learner_dir . $target_filename;
                            
                            // Check if file already exists
                            if (!file_exists($target_path)) {
                                if (copy($source_path, $target_path)) {
                                    $generatedFiles[] = $target_path;
                                    log_message("Copied $doc_name to $target_path from $source_path");
                                } else {
                                    log_message("Failed to copy $doc_name from $source_path to $target_path");
                                }
                            } else {
                                log_message("$doc_name already exists at $target_path, skipping");
                            }
                        } else {
                            log_message("$doc_name file not found for LearnerID {$data['LearnerID']}, searched paths: " . implode(', ', array_slice($possible_paths, 0, 10)));
                        }
                    } else {
                        log_message("No $doc_name record found for LearnerID {$data['LearnerID']}");
                    }
                } else {
                    log_message("Failed to prepare document query for $doc_name, LearnerID {$data['LearnerID']}: " . $conn->error);
                }
            }
...
if (!empty($generatedFiles)) {
    if (count($learners) === 1) {
        if (count($generatedFiles) === 1 && !$forceZip) {
        ...
        try {
            ...
            if (file_exists($zipFileName)) {
                ...
                copyToDesktop($zipFileName, $desktop_dir, $desktop_enabled);
                while (ob_get_level()) { ob_end_clean(); }
                header('Content-Type: application/zip');
                header('Content-Disposition: attachment; filename="' . basename($zipFileName) . '"');
                readfile($zipFileName);
