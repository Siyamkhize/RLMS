...
class LearnerInductionPage extends StatefulWidget {
  final String learnerID;
  final String learnerName;
  final String classID;

  const LearnerInductionPage({
    super.key,
  ...
  @override
  State<LearnerInductionPage> createState() => _LearnerInductionPageState();
}

class _LearnerInductionPageState extends State<LearnerInductionPage> {
  ...
  Future<void> _initializeData() async {
    // Fast initialization for single learner - skip unnecessary operations
    print('Fast init for single learner: ${widget.learnerID}');

    // Load only the specific learner data
    await _loadSingleLearnerFromDatabase();
    await _checkLearnerClockingStatus(widget.learnerID);

    // Start periodic refresh (reduced frequency for single learner)
  ...
  Future<void> _checkLearnerClockingStatus(String learnerId) async {
    ...
    // Fast single query for specific learner
  ...
  void _startPeriodicRefresh() {
    // Reduced refresh frequency for single learner (10 seconds instead of 5)
    Future.delayed(const Duration(seconds: 10), () {
      if (mounted) {
        _refreshSingleLearnerData();
        _startPeriodicRefresh();
      }
    });
  }
  ...
  Future<void> _fetchClockingDataFromServer() async {
    // Skip server fetch for single learner - induction is one-time only
    // This prevents unnecessary network calls and speeds up the page
    print('[INDUCTION] Skipping server fetch - single learner induction mode');
    return;
  }
  ...
  Future<void> _refreshSingleLearnerData() async {
    try {
      // Fast refresh for single learner only
      await _checkLearnerClockingStatus(widget.learnerID);

      // Quick sync check (no heavy operations)
      if (await _checkConnectivity()) {
        await _syncOfflineClockIns();
      }
    } catch (e) {
      print('Error refreshing single learner data: $e');
    }
  }

  Future<void> _loadSingleLearnerFromDatabase() async {
    try {
      final dbHelper = DatabaseHelper();
      final db = await dbHelper.database;

      // Direct query for single learner - much faster than loading all learners
      final learnerData = await db.rawQuery('''
        SELECT l.*, ic.clock_in_time, ic.clock_out_time, ic.contact_time
        FROM learnerdetails l
        LEFT JOIN induction_clocking ic ON l.LearnerID = ic.LearnerID
        WHERE l.LearnerID = ? AND l.ClassID = ?
        LIMIT 1
      ''', [widget.learnerID, widget.classID]);

      if (learnerData.isNotEmpty) {
        final learner = learnerData.first;
        ...
        // Store clocking times
        if (clockInTime.isNotEmpty && clockInTime != 'null') {
          clockInTimes[learnerId] = clockInTime;
        }
        if (clockOutTime.isNotEmpty && clockOutTime != 'null') {
          clockOutTimes[learnerId] = clockOutTime;
        }
        if (contactTime.isNotEmpty && contactTime != 'null') {
      ...
      print('Error loading single learner: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Error loading learner data: $e')),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: [
        Scaffold(
          appBar: AppBar(
            title: Text('Learner Induction: ${widget.learnerName}'),
            ...
            actions: [
              IconButton(
                ...
                onPressed: () async {
                  // Fast sync for single learner
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('Refreshing learner data...'),
                      backgroundColor: Colors.blue,
                      duration: Duration(milliseconds: 500),
                    ),
                  );

                  // Quick refresh without heavy operations
                  try {
                    await _loadSingleLearnerFromDatabase();
                    await _checkLearnerClockingStatus(widget.learnerID);
                    ...
                    if (mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text('Learner data refreshed'),
                          backgroundColor: Colors.green,
          ...
          body: Padding(
            ...
            child: Column(
              ...
              children: [
                Text(
                  'Learner Induction Details',
                  style: const TextStyle(
                      fontSize: 18, fontWeight: FontWeight.bold),
                ),
                const SizedBox(height: 8),
                Text(
                  'Learner: ${widget.learnerName} (ID: ${widget.learnerID}) | Class: ${widget.classID}',
                  style: const TextStyle(
                      fontSize: 14,
                ...
                const Text(
                  'Biometric Authentication: Clock In/Out',
                ...
                learners.isEmpty
                    ? const Center(
                        child: Text(
                            'Learner not found or not enrolled in this class'))
                    : Expanded(
                          ...
                          child: SingleChildScrollView(
                            ...
                            child: DataTable(
                              ...
                              rows: learners.map((item) {
                                ...
                                // Show the specific learner (no highlighting needed since it's the only one)
                                bool isSearchedLearner =
                                    learnerID == widget.learnerID;

                                return DataRow(
                                  color: isSearchedLearner
