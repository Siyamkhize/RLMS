...
class _LearnerDetailsPageState extends State<LearnerDetailsPage>
  ...
  bool _shouldUseDropdown(String fieldName) {
    // Only use dropdown if the field is in our dropdown options
    return _dropdownOptions.containsKey(fieldName);
  }
          ...
          if (jsonResponse['success'] == true) {
            setState(() {
              ...
              learnerData!.forEach((key, value) {
                ...
                // If this is a dropdown field, normalize to the exact option from our list
                if (_dropdownOptions.containsKey(key) && finalValue != null) {
                  final trimmedValue = finalValue.trim();
                  final options = _dropdownOptions[key] ?? [];
                  // Find case-insensitive match
                  String? matchedOption;
                  for (final option in options) {
                    if (option.toLowerCase() == trimmedValue.toLowerCase()) {
                  ...
                  if (matchedOption != null) {
                    ...
                    learnerData![key] =
                        matchedOption; // Also update the learnerData map
                  } else {
                    // If no match, clear the value to avoid dropdown errors
                    finalValue = null;
                    learnerData![key] = null;
                  }
                }
                _controllers[key] =
                    TextEditingController(text: finalValue ?? '');
              });
      ...
      if (localLearnerData != null) {
        setState(() {
          ...
          learnerData!.forEach((key, value) {
            ...
            _controllers[key] = TextEditingController(text: finalValue ?? '');
          });
          // Set up controller listeners after controllers are initialized
          _addControllerListeners();
        });
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (shouldUseDropdown) ...[
                    // Dropdown widget with clear button
                    Row(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Expanded(
                          child: DropdownButtonFormField<String>(
                            value: () {
                              // Use learnerData as source of truth (stable across rebuilds)
                              final options = _dropdownOptions[entry.key] ?? [];
                              final raw =
                                  learnerData![entry.key]?.toString() ?? '';
                            ...
                            items: _dropdownOptions[entry.key]!
                                .map((String value) {
                              ...
                              if (newValue != null) {
                                setState(() {
                                  ...
                                  learnerData![entry.key] =
                                      newValue; // Keep source of truth in sync
                                });
                              }
                            },
