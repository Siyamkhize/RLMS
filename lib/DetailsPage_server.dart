...
class _POETabContentState extends State<POETabContent> {
  ...
  Future<void> _saveLocally(File document, String assessmentType,
    ...
    try {
      ...
      if (unitStandard != null && unitStandard.isNotEmpty) {
        ...
        if (match != null) {
          ...
          String digits = unitStandard.replaceAll(RegExp(r'[^0-9]'), '');
      ...
      final newFileName =
          '${assessmentType}_${unitStandardId}_${sanitizedExercise}_$timestamp.$extension';
    ...
    // Save to local database with unitStandard
    ...
    setState(() {
      // Add both old and new keys for compatibility!
      final oldKey = '$assessmentType-$exercise-${widget.learnerID}';
      uploadedExercises[oldKey] = true;

      if (unitStandard != null) {
        // Generate the new-style key too, using the same _uploadKey logic
        String normalizedType = assessmentType;
        if (assessmentType.toLowerCase() == 'formative')
          normalizedType = 'Formative';
        if (assessmentType.toLowerCase() == 'summative')
          normalizedType = 'Summative';
        if (assessmentType.toLowerCase() == 'logbook')
          normalizedType = 'LogBook';
        if (assessmentType.toLowerCase() == 'formativeremedial')
          normalizedType = 'FormativeRemedial';
        if (assessmentType.toLowerCase() == 'summativeremedial')
          normalizedType = 'SummativeRemedial';

        final normalizedExercise =
            exercise.replaceAll(RegExp(r'\s+'), '').toLowerCase();
        final normalizedUnit = unitStandard.trim();
        final newKey =
            '$normalizedType-$normalizedExercise-$normalizedUnit-${widget.learnerID}';
        uploadedExercises[newKey] = true;
      }
    });
