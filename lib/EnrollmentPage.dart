...
class EnrollmentPage extends StatefulWidget {
  ...
  const EnrollmentPage({Key? key, required this.learnerId, this.returnToClockAfterEnroll = false}) : super(key: key);
  ...
  void _setupStreamListeners() {
    ...
    _fingerprintService.enrollSuccessStream.listen((result) async {
      ...
      try {
        ...
        debugPrint('[ENROLL] ZKTeco template preview: ${template.toString().length > 0 ? template.toString().substring(0, template.toString().length > 50 ? 50 : template.toString().length) : 'EMPTY'}');
