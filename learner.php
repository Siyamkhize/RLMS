...
// Security: Hide PHP version and server information
header('X-Powered-By: ');
header('Server: ');

// Security: Prevent caching to avoid URL exposure
header('Cache-Control: no-store, no-cache, must-revalidate, max-age=0');
header('Pragma: no-cache');
header('Expires: 0');

// Security: Content Security Policy
header("Content-Security-Policy: default-src 'self' 'unsafe-inline' 'unsafe-eval' https://maxcdn.bootstrapcdn.com; frame-ancestors 'none';");
...
<head>
    ...
    <title>Learner Attendance Report</title>
    <style>
    * {
    margin: 0;
    padding: 0;
...
@media (max-width: 480px) {
    ...
    .outstanding-fees-message {
        padding: 10px !important;
        margin: 10px 0 !important;
    }

    .outstanding-fees-message strong {
        font-size: 16px !important;
    }

    .outstanding-fees-message p {
...
<body>
        ...
        <div class="container">
            ...
            <?php if ($showOutstandingFeesMessage): ?>
                <div class="outstanding-fees-message" style="background: #fff3cd; border: 2px solid #ffc107; padding: 15px; margin: 20px 0; border-radius: 8px; text-align: center;">
                    ...
                    <p style="color: #856404; margin: 10px 0 0 0;">October stipends are still pending and haven't been processed yet.</p>
                </div>
            <?php endif; ?>
            ...
            <?php else: ?>
                ...
                <?php else: ?>
                    <div class="no-data" style="text-align: center; padding: 40px; background: #fff3cd; border: 1px solid #ffeaa7; border-radius: 8px; margin: 20px 0;">
                        <?php if (!empty($idSearch)): ?>
                            <h3 style="color: #856404;">No active learner data found for this ID Number</h3>
                            <p style="color: #856404; font-size: 16px;">
                                ID Number: <strong><?php echo htmlspecialchars($idSearch); ?></strong>
                            </p>
                        <?php else: ?>
                            <h3 style="color: #6c757d;">No active learner data found for the selected criteria.</h3>
                            <p style="color: #6c757d;">Please adjust your filters and try again.</p>
                            <?php if (!empty($attendanceFilters)): ?>
                                <p style="color: #856404;"><em>Note: Your attendance filters may be too restrictive. Try adjusting the attendance criteria.</em></p>
                            <?php endif; ?>
                        <?php endif; ?>
                    </div>
                <?php endif; ?>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            // Security: Enhanced paste prevention for ALL devices (desktop, mobile, tablet)
            const idSearchInput = document.getElementById('id_search');
            if (idSearchInput) {
                // Prevent paste event
                idSearchInput.addEventListener('paste', function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                    alert('Pasting is disabled in this field. Please type the ID number manually.');
                    return false;
                });

                // Prevent input event (for mobile paste)
                idSearchInput.addEventListener('input', function(e) {
                    // Check if input was pasted (length changed significantly)
                    const currentValue = this.value;
                    if (currentValue.length > 1 && !this.dataset.lastLength) {
                        // Likely a paste operation
                        this.value = '';
                        alert('Pasting is disabled. Please type manually.');
                    }
                    this.dataset.lastLength = currentValue.length;
                });

                // Prevent drop event (drag and drop)
                idSearchInput.addEventListener('drop', function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                    alert('Dropping text is disabled. Please type manually.');
                    return false;
                });

                // Prevent dragover
                idSearchInput.addEventListener('dragover', function(e) {
                    e.preventDefault();
                    return false;
                });

                // Disable context menu on this specific input
                idSearchInput.addEventListener('contextmenu', function(e) {
                    e.preventDefault();
                    return false;
                });

                // Disable long press on mobile (iOS/Android)
                idSearchInput.addEventListener('touchstart', function(e) {
                    this.dataset.touchStartTime = Date.now();
                });

                idSearchInput.addEventListener('touchend', function(e) {
                    const touchDuration = Date.now() - (this.dataset.touchStartTime || 0);
                ...
                // Disable autocomplete and autofill for security
                idSearchInput.setAttribute('autocomplete', 'off');
                idSearchInput.setAttribute('autocorrect', 'off');
                idSearchInput.setAttribute('autocapitalize', 'off');
                idSearchInput.setAttribute('spellcheck', 'false');
                
                // Prevent iOS/Android paste menu
                idSearchInput.style.webkitUserSelect = 'none';
                idSearchInput.style.userSelect = 'none';
            }
