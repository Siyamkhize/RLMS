...
try {
    ...
    // FIXED: Updated logClockingAttempt function to include all required fields for clocking_log table
    function logClockingAttempt($conn, $learnerID, $reason, $userLatitude = null, $userLongitude = null, $userAccuracy = null, $siteLatitude = null, $siteLongitude = null, $action = 'clock_out') {
        // Ensure all values have defaults to prevent SQL errors
        $learnerID = $learnerID !== null ? $learnerID : '0';
        $reason = $reason !== null ? $reason : 'Unknown reason';
        $action = $action !== null ? $action : 'clock_out';
        $userLatitude = $userLatitude !== null ? (string)$userLatitude : '0.0';
        $userLongitude = $userLongitude !== null ? (string)$userLongitude : '0.0';
        $userAccuracy = $userAccuracy !== null ? (string)$userAccuracy : '0.0';
        $siteLatitude = $siteLatitude !== null ? (string)$siteLatitude : '0.0';
        $siteLongitude = $siteLongitude !== null ? (string)$siteLongitude : '0.0';

        $stmt = $conn->prepare("INSERT INTO clocking_log (learnerID, action, attempt_time, user_latitude, user_longitude, accuracy, site_latitude, site_longitude, reason) VALUES (?, ?, NOW(), ?, ?, ?, ?, ?, ?)");
        if (!$stmt) {
    ...
    function calculateContactTime($clockInTime, $clockOutTime, $currentDate) {
        try {
            ...
            if ($clockIn === false) {
                ...
                $clockIn->setDate(intval(substr($currentDate, 0, 4)), intval(substr($currentDate, 5, 2)), intval(substr($currentDate, 8, 2)));
            }
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        if (isset($_POST['clock_out'])) {
            ...
            // FIXED: Extract location data from POST with defaults
            ...
            $userAccuracy = 50.0; // Default accuracy
            ...
            if (empty($learnerID) || empty($classID)) {
                ...
                logClockingAttempt($conn, $learnerID, $reason, $userLatitude, $userLongitude, $userAccuracy);
            ...
            // Update clock-out time and contact time
            ...
            file_put_contents('debug_clockout.log', "Updated learner_clocking: clock_out_time=$currentTime, contact_time=$contactTime, synced=$isSynced for learnerID=$learnerID, date=$currentDate" . PHP_EOL, FILE_APPEND);
            $stmt->close();
            ...
            // FIXED: Include location data in successful clock-out log
            logClockingAttempt($conn, $learnerID, "Successful clock-out", $userLatitude, $userLongitude, $userAccuracy);
