...
// Simple logging functions to replace missing clocking_debug_logger.php
function logClockingEvent($level, $event, $data) {
    $logEntry = date('Y-m-d H:i:s') . " - [$level] $event - Data: " . json_encode($data) . PHP_EOL;
    file_put_contents('debug_clockin.log', $logEntry, FILE_APPEND);
}

function logClockIn($learnerID, $source, $data) {
    $logEntry = date('Y-m-d H:i:s') . " - Clock-in attempt - LearnerID: $learnerID, Source: $source, Data: " . json_encode($data) . PHP_EOL;
    file_put_contents('debug_clockin.log', $logEntry, FILE_APPEND);
}

function logDbUpdate($table, $data) {
    $logEntry = date('Y-m-d H:i:s') . " - DB Update - Table: $table, Data: " . json_encode($data) . PHP_EOL;
    file_put_contents('debug_clockin.log', $logEntry, FILE_APPEND);
}

function logAutoClockOut($learnerID, $reason, $data) {
    $logEntry = date('Y-m-d H:i:s') . " - Auto Clock-out - LearnerID: $learnerID, Reason: $reason, Data: " . json_encode($data) . PHP_EOL;
    file_put_contents('debug_clockin.log', $logEntry, FILE_APPEND);
}

// Simple database state checker (replaces missing clockingLogger)
function getCurrentDatabaseState($conn, $learnerID, $currentDate) {
    try {
        ...
        $stmt->close();
        return $data ? ['learner_clocking' => $data] : null;
    } catch (Exception $e) {
        return null;
    }
}
...
try {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $beforeState = getCurrentDatabaseState($conn, $learnerID, $currentDate);
        ...
        if ($result->num_rows > 0) {
            ...
            if (!empty($existingRecord['clock_in_time'])) {
                ...
                $response['clock_in_time'] = $existingRecord['clock_in_time'];
                $response['clock_out_time'] = $existingRecord['clock_out_time'];
                $response['contact_time'] = $existingRecord['contact_time'];
                $stmt->close();
                echo json_encode($response);
                exit;
            }
        ...
        // Insert or update clock-in record - FIXED: Added user_accuracy parameter
        ...
        // FIXED: Added 'd' for user_accuracy parameter
        ...
        if ($stmt->execute()) {
            ...
            $afterState = getCurrentDatabaseState($conn, $learnerID, $currentDate);
