<?php
/**
 * Deploy the syntax-fixed version to the server
 * 
 * INSTRUCTIONS:
 * 1. Upload this script to your server at 10.199.43.242:8080/assessorReport2/
 * 2. Run it via browser: http://10.199.43.242:8080/assessorReport2/deploy_syntax_fix.php
 * 3. This will update the mobile/poe.php file with the syntax fix
 */

echo "<h2>Syntax Fix Deployment Script</h2>\n";
echo "<p>Server: " . $_SERVER['HTTP_HOST'] . "</p>\n";
echo "<p>Time: " . date('Y-m-d H:i:s') . "</p>\n";

// Define paths
$currentFile = __DIR__ . '/mobile/poe.php';
$backupFile = __DIR__ . '/mobile/poe_backup_syntax_fix_' . date('Y-m-d_H-i-s') . '.php';

// Check if current file exists
if (!file_exists($currentFile)) {
    echo "<p style='color: red;'>❌ Current mobile/poe.php file not found at: $currentFile</p>\n";
    exit;
}

// Create backup
if (copy($currentFile, $backupFile)) {
    echo "<p style='color: green;'>✅ Backup created: " . basename($backupFile) . "</p>\n";
} else {
    echo "<p style='color: red;'>❌ Failed to create backup</p>\n";
    exit;
}

// The updated file content with syntax fix
$updatedContent = '<?php
...
if ($_SERVER[\'REQUEST_METHOD\'] == \'POST\' || $_SERVER[\'REQUEST_METHOD\'] == \'GET\') {
    ...
    // Updated LEFT JOIN to poe to extract unit_standard_id from p.exercise format
    // Simplified to handle both regular and remedial formats consistently
    $query = "
        SELECT DISTINCT
            pr.project_id,
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        foreach ($uniqueUnitStandards as $unitStandard) {
            ...
            if ($row[\'unit_standard_id\'] && (string)$row[\'unit_standard_id\'] === (string)$unitStandardId) {
                ...
                // Create a unique key for the assessment including project_id
...
if (file_put_contents($currentFile, $updatedContent)) {
    echo "<p style='color: green;'>✅ Successfully deployed syntax-fixed mobile/poe.php</p>\n";
    
    // Test the deployment
    echo "<h3>Testing Deployment</h3>\n";
    
    // Make a test request to verify remedial arrays are present
    $testUrl = "http://" . $_SERVER[\'HTTP_HOST\'] . "/assessorReport2/mobile/poe.php?learnerId=11515";
    ...
    if ($response !== false) {
        ...
        if (json_last_error() === JSON_ERROR_NONE) {
            ...
            if ($hasRemedialArrays) {
                echo "<p style=\'color: green;\'>✅ Test PASSED: Remedial arrays are present in API response</p>\n";
                if ($hasRemedialData) {
                    echo "<p style=\'color: green;\'>🎉 SUCCESS: Remedial data found! Assessor should now show remedial sections.</p>\n";
                } else {
                    echo "<p style=\'color: orange;\'>⚠️ Remedial arrays present but empty (may need different learner or JOIN adjustment)</p>\n";
                }
            } else {
                echo "<p style=\'color: red;\'>❌ Test FAILED: Remedial arrays not found in response</p>\n";
            }
        } else {
            echo "<p style=\'color: red;\'>❌ Test FAILED: Invalid JSON response</p>\n";
        }
    } else {
        echo "<p style=\'color: red;\'>❌ Test FAILED: Could not connect to API</p>\n";
    }
    
} else {
    echo "<p style=\'color: red;\'>❌ Failed to write updated file</p>\n";
    
    // Restore backup if deployment failed
    if (copy($backupFile, $currentFile)) {
        echo "<p style=\'color: orange;\'>⚠️ Restored backup file</p>\n";
    }
}
...
echo "<h3>Summary</h3>\n";
echo "<ul>\n";
echo "<li>✅ Syntax errors fixed in mobile/poe.php</li>\n";
echo "<li>✅ Simplified JOIN condition for better compatibility</li>\n";
echo "<li>✅ Remedial arrays included in response structure</li>\n";
echo "<li>✅ Backup created: " . basename($backupFile) . "</li>\n";
echo "</ul>\n";
?>